...
import { provideEditorFlags, useEditorFlags } from '../../composables/useEditorFlags.ts'

const ChildComponent = defineComponent({
	template: '<span id="child-flags">{{ { isPublic, isRichEditor, isRichWorkspace } }}</span>',
	setup() {
...
const ParentComponent = defineComponent({
	template: '<div><span id="flags">{{ { isPublic, isRichEditor, isRichWorkspace } }}</span><slot /></div>',
...
test('provides and injects editor flags', () => {
	...
	expect(parentFlags).toEqual({ isPublic: true, isRichEditor: false, isRichWorkspace: false })
