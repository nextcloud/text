...
/**
 * markdown-it plugin: parse Obsidian-style wiki image links `![[filename]]`.
 *
 * Produces a standard `image` token so that downstream plugins
 * (e.g. markdown-it-image-figures) treat it exactly like a normal image.
 * A `data-wiki-image` attribute is set so the ProseMirror serializer can
 * round-trip the syntax back to `![[filename]]` instead of `![alt](src)`.
 */
