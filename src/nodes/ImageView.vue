...
<template>
	<NodeViewWrapper :contenteditable="isEditable">
		<figure
			...
			<div
				v-else-if="canDisplayPlaceholder"
				:style="{ width: imageWidth + 'px', height: imageHeight + 'px' }"
				class="image__placeholder">
				<canvas
					v-if="hasBlurhash"
					ref="canvas"
					class="image__blurhash"
					aria-hidden="true" />
			</div>
...
export default {
	...
	computed: {
		...
		hasBlurhash() {
			return this.imageBlurhash !== null
		},
		src: {
			get() {
	...
	methods: {
		...
		async loadAttachmentMetadata() {
			try {
				...
				// wait for next tick to have the canvas in the DOM for blurhash drawing
				await this.$nextTick()
				this.drawBlurhash()
			} catch (err) {
		...
		drawBlurhash() {
			// per implementation in photos > File.vue
