...
describe('Test state loading of documents', function () {
	...
	it('Initial content can not be undone', function () {
		...
		cy.getContent().type('New content')
	...
	it('Consecutive sessions work properly', function () {
		...
		cy.logout()
		cy.get('@readToken').then((token) => cy.visit(`/s/${token}`))
		cy.wait('@create')
		...
		cy.getContent()
			...
			.find('h2')
			.should('contain', 'Hello world')
		cy.get('@readToken').then(cy.closeInterceptedSession)
		...
		cy.login(user)
		cy.shareFile('/test2.md', { edit: true }).as('writeToken')
		// Open write link and edit something
		cy.get('@writeToken').then((token) => cy.visit(`/s/${token}`))
		cy.getEditor().should('be.visible')
		cy.getContent()
			.should('contain', 'Hello world')
			.find('h2')
			.should('contain', 'Hello world')
		cy.getContent().type('Something new {end}')
		cy.intercept({ method: 'POST', url: '**/session/*/sync' }).as('sync')
		cy.wait('@sync')
		cy.get('@writeToken').then(cy.closeInterceptedSession)
	...
	it('Load after state has been saved', function () {
		...
		cy.getContent()
			.should('contain', 'Hello world')
			.find('h2')
			.should('contain', 'Something new Hello world')

		cy.login(user)
		cy.shareFile('/test3.md').as('readToken')
		cy.logout()
		cy.get('@readToken').then((token) => cy.visit(`/s/${token}`))
