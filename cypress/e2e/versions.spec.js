...
describe('Versions', () => {
	...
	it('View versions with close timestamps', () => {
		cy.isolateTest().then(({ folderName, fileName }) => {
			...
			cy.createFile(fullPath, '# V1', 'text/markdown', {
				'x-oc-mtime': 1691420501,
			})
			cy.createFile(fullPath, '# V2', 'text/markdown', {
				'x-oc-mtime': 1691420521,
			})
	...
	it('Compare versions', () => {
		cy.isolateTest().then(({ folderName, fileName }) => {
			...
			cy.get('[data-files-versions-versions-list] li')
				.eq(2)
