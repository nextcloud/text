...
describe('Open test.md in viewer', function () {
	...
	it('Open the viewer on file click', function () {
		...
		getViewer().find('.modal-header__name').should('contain', 'test.md')
		getViewer()
		...
		cy.getContent().get('h2').should('contain', 'Hello world')
	...
	it('Open an empty file', function () {
		...
		getViewer()
			...
			.and('have.class', 'modal-mask')
			.and('not.have.class', 'icon-loading')
		getViewer().find('.modal-header__name').should('contain', 'empty.md')
		getViewer()
			.find('.modal-header button.action-item__menutoggle')
			.should('be.visible')
	...
	it('Can use tab keys for list in the viewer', function () {
		...
		cy.getContent().trigger('keydown', { keyCode: 9 })
		cy.getContent().trigger('keyup', { keyCode: 9 })
		cy.getContent().type('Nested list')

		cy.getContent().find('ul li').should('contain', 'test')
		cy.getContent().find('ul li ul li').should('contain', 'Nested list')

		cy.getContent().trigger('keydown', { keyCode: 9, shiftKey: true })
		cy.getContent().trigger('keyup', { keyCode: 9, shiftKey: true })
