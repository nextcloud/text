...
describe('Smart picker', () => {
	...
	it('Type and see the smart picker', () => {
		...
		cy.get('.tippy-box .suggestion-list')
			.children()
			.should(($children) => {
				const entries = $children
					.find('.suggestion-list__item')
		...
		cy.getContent().type('Heading{enter}Hello World{enter}')

		cy.getContent().find('h1').should('contain.text', 'Hello World')
	})
	...
	it('Insert a link with the smart picker', () => {
		...
		cy.get('.reference-picker input').type('https://github.com')

		cy.get('.reference-widget')
			.should('be.visible')
			.should('contain.text', 'GitHub')

		cy.get('.reference-picker input').type('{enter}')

		cy.getContent().find('a').should('contain.text', 'https://github.com')
	})
})
