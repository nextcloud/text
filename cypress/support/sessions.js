...
Cypress.Commands.add(
	...
	({ connection, steps, version, awareness = '' }) => {
		return push(connection, { steps, version, awareness }).then((response) => response.data)
	},
)
...
Cypress.Commands.add('syncSteps', (connection, options = { version: 0 }) => {
	return sync(connection, options).then((response) => response.data)
})
...
Cypress.Commands.add('save', (connection, options = { version: 0 }) => {
	return save(connection, options).then((response) => response.data)
})
...
Cypress.Commands.add(
	'pushAndClose',
	({ connection, steps, version, awareness = '' }) => {
		cy.log('Race between push and close').then(() => {
			const pushed = push(connection, { steps, version, awareness }).catch((error) => {
