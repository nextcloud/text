...
Cypress.Commands.add(
	'createFile',
	(target, content, mimeType = 'text/markdown', headers = {}) => {
		const blob = new Blob([content], { type: mimeType })
		return axios
			.put(`${url}/remote.php/webdav/${target}`, blob.size > 0 ? blob : '', {
...
Cypress.Commands.add('createMarkdown', (fileName, content, reload = true) => {
	return cy.createFile(fileName, content, 'text/markdown').then((fileId) => {
		if (reload) {
			cy.reloadFileList()
		}
...
Cypress.Commands.add('shareFileToUser', (path, targetUser, shareData = {}) => {
	Cypress.log()
	return axios.post(`${url}/ocs/v2.php/apps/files_sharing/api/v1/shares`, {
		path,
		shareType: 0,
...
Cypress.Commands.add(
	'isolateTest',
	({ sourceFile = 'empty.md', targetFile = null, onBeforeLoad } = {}) => {
		targetFile = targetFile || sourceFile
		cy.createTestFolder().then((folderName) => {
			cy.uploadFile(
				sourceFile,
				'text/markdown',
				`${encodeURIComponent(folderName)}/${targetFile}`,
			)
			window.__currentDirectory = folderName
			return cy
				.visitTestFolder({ onBeforeLoad })
...
Cypress.Commands.add('shareFile', (path, options = {}) => {
	...
	return axios
		.post(`${url}/ocs/v2.php/apps/files_sharing/api/v1/shares`, {
			path,
			shareType,
		})
		...
		.then(({ token, id }) => {
			...
			return axios
				.put(`${url}/ocs/v2.php/apps/files_sharing/api/v1/shares/${id}`, {
					permissions,
				})
				.then(() => token)
		})
...
Cypress.Commands.add('waitForPreview', (name) => {
	cy.getFile(name).scrollIntoView({ offset: { top: -200 } })
	cy.getFile(name)
...
Cypress.Commands.add('getFileContent', (path) => {
	return axios
		.get(`${url}/remote.php/webdav/${path}`)
...
Cypress.Commands.add('openFolder', (name) => {
	...
	cy.intercept({ method: 'PROPFIND', url }).as(`open-${name}`)
	cy.openFile(name)
	cy.wait(`@open-${name}`)
})
...
Cypress.Commands.add('closeFile', (params = {}) => {
	cy.intercept({ method: 'POST', url: '**/apps/text/session/*/close' }).as('close')
...
Cypress.Commands.add('closeInterceptedSession', (shareToken = undefined) => {
	return axios.post(closeData.url, {
...
Cypress.Commands.add('getContent', { prevSubject: 'optional' }, (subject) => {
	return (subject ? cy.wrap(subject) : cy.getEditor()).find('.ProseMirror', {
		timeout: 10000,
	})
})
...
Cypress.Commands.add('openWorkspace', () => {
	...
	cy.getEditor()
		.find('[data-text-el="editor-content-wrapper"]')
		.click({ force: true })
...
Cypress.Commands.add('configureText', (key, value) => {
	return axios.post(`${url}/index.php/apps/text/settings`, { key, value })
})

Cypress.Commands.add('showHiddenFiles', (value = true) => {
	Cypress.log()
	return axios.put(`${url}/index.php/apps/files/api/v1/config/show_hidden`, {
		value,
	})
})
...
Cypress.Commands.add('setAppConfig', (key, value) => {
	Cypress.log()
	return axios.post(`${url}/ocs/v2.php/apps/testing/api/v1/app/text/${key}`, {
		value,
	})
})
...
Cypress.Commands.add(
	'createDescription',
	(buttonLabel = 'Add folder description') => {
