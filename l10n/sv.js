OC.L10N.register(
    "text",
    {
    "The file was uploaded" : "Filen har laddats upp",
    "The uploaded file exceeds the upload_max_filesize directive in php.ini" : "Den uppladdade filen är större än vad som tillåts av upload_max_filesize i php.ini",
    "The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form" : "Den uppladdade filen överstiger MAX_FILE_SIZE-direktivet som specificerades i HTML-formuläret",
    "The file was only partially uploaded" : "Filen laddades bara upp delvis",
    "No file was uploaded" : "Ingen fil laddades upp",
    "Missing a temporary folder" : "Saknar en tillfällig mapp",
    "Could not write file to disk" : "Det gick inte att skriva filen till disken",
    "A PHP extension stopped the file upload" : "Ett PHP-tillägg stoppade filuppladdningen",
    "No file uploaded or file size exceeds maximum of %s" : "Ingen fil uppladdad eller filstorleken överstiger max %s",
    "File not found" : "Filen hittades inte",
    "Nextcloud Text" : "Nextcloud Text",
    "Text document" : "Textdokument",
    "Create new text file" : "Skapa ny textfil",
    "Editing session has expired. Please reload the page." : "Redigeringssessionen har löpt ut. Läs in sidan igen.",
    "{user} has mentioned you in the text document {node}" : "{user} har nämnt dig i textdokument {node}",
    "This file cannot be displayed as download is disabled by the share" : "Den här filen kan inte visas eftersom hämtning har inaktiverats för delningen.",
    "This document is read-only." : "Det här dokumentet är skrivskyddat.",
    "Read-only permission cannot save document changes. Please reload the page." : "Med skrivskyddad behörighet går det inte att spara ändringar i dokumentet. Läs in sidan igen.",
    "Readme" : "Readme",
    "Text" : "Text",
    "📝 Collaborative document editing" : "Gemensam dokumentredigering",
    "** 📝 Collaborative document editing!**\n\n- **📝 Focused writing:** No distractions, only the formatting you need.\n- **🙋 Work together:** Share and collaborate with friends and colleagues, no matter if they use Nextcloud or not!\n- **💾 Open format:** Files are saved as [Markdown](https://en.wikipedia.org/wiki/Markdown), so you can edit them from any other text app too.\n- **✊ Strong foundation:** We use [🐈 tiptap](https://tiptap.scrumpy.io) which is based on [🦉 ProseMirror](https://prosemirror.net) – huge thanks to them!" : "** 📝 Gemensam dokumentredigering!**\n\n- **📝 Fokuserat skrivande:** Inga distraktioner, endast den formatering du behöver.\n- **🙋 Arbeta tillsammans:** Dela och samarbeta med vänner och kollegor, oavsett om de använder Nextcloud eller inte!\n- **💾 Öppna format:** Filer sparas som [Markdown](https://en.wikipedia.org/wiki/Markdown), så du kan redigera dem från valfri text-app också.\n- **✊ Stark grund:** Vi använder [🐈 tiptap](https://tiptap.scrumpy.io) som baseras på [🦉 ProseMirror](https://prosemirror.net) – tack till dem!",
    "Your editing permissions have been revoked. The document is now read-only." : "Din redigeringsbehörighet har återkallats. Dokumentet är nu skrivskyddat.",
    "You now have edit permissions for this document." : "Du har nu redigeringsbehörighet för dokumentet.",
    "Keep my local changes and overwrite the remote version" : "Behåll mina lokala ändringar och skriv över serverversionen",
    "Discard my local changes and use the remote version" : "Ignorera mina lokala ändringar och använd serverversionen",
    "Comments" : "Kommentarer",
    "Previous comment" : "Föregående kommentar",
    "Next comment" : "Nästa kommentar",
    "Close" : "Stäng",
    "Guest" : "Gäst",
    "guest" : "gäst",
    "Edit" : "Redigera",
    "Delete" : "Ta bort",
    "Save" : "Spara",
    "Cancel" : "Avbryt",
    "Enter your name to comment" : "Ange ditt namn för att kommentera",
    "Add a comment…" : "Lägg till en kommentar…",
    "Comment" : "Kommentar",
    "Reply" : "Svara",
    "Front matter changed" : "Inledande metadata ändrades",
    "Image changed" : "Bild ändrades",
    "Mention changed" : "Omnämnande ändrades",
    "Mathematics changed" : "Matematik ändrades",
    "Footnote changed" : "Fotnot ändrades",
    "Divider changed" : "Avgränsare ändrades",
    "Changed content" : "Innehållet ändrades",
    "Paragraph" : "Stycke",
    "Heading" : "Rubrik",
    "List item" : "Listobjekt",
    "Task" : "Uppgifter",
    "Table" : "Tabell",
    "Table row" : "Tabellrad",
    "Table cell" : "Tabellcell",
    "Image" : "Bild",
    "Code block" : "Kodblock",
    "Quote" : "Offert",
    "Callout" : "Informationsruta",
    "Details" : "Detaljer",
    "Footnote" : "Fotnot",
    "Footnote reference" : "Fotnotsreferens",
    "Front matter" : "Förtexter",
    "Mathematics" : "Matematik",
    "Mention" : "Omnämnande",
    "Link preview" : "Länkförhandsvisning",
    "Added" : "Tillagd",
    "Removed" : "Borttagen",
    "Changed" : "Ändrat",
    "Moved" : "Flyttades",
    "Content added" : "Innehåll lades till",
    "Content removed" : "Innehåll togs bort",
    "Content changed" : "Innehållet ändrades",
    "No content" : "Inget innehåll",
    "Block-level" : "På blocknivå",
    "Moved section" : "Avsnitt flyttades",
    "Structure changed" : "Strukturen ändrades",
    "{context} added" : "{context} lades till",
    "{context} removed" : "{context} togs bort",
    "{context} changed" : "{context} ändrades",
    "Table column added" : "Tabellkolumn lades till",
    "Table column removed" : "Tabellkolumn togs bort",
    "Table column changed" : "Tabellkolumn ändrades",
    "Section: {section}" : "Avsnitt: {section}",
    "Change pages" : "Ändra sidor",
    "Previous" : "Föregående",
    "Showing changes {from}–{to} of {total}" : "Visar ändringar {from}–{to} av {total}",
    "Next" : "Nästa",
    "Document start" : "Dokumentets början",
    "Block-level change; fine inline detail is unavailable" : "Ändring på blocknivå; detaljer på radnivå är inte tillgängliga",
    "_%n change_::_%n changes_" : ["%n ändring","%n ändringar"],
    "_%n edit_::_%n edits_" : ["%n redigering","%n redigeringar"],
    "Reload" : "Ladda om",
    "The file was overwritten. Your current changes cannot be auto-saved. Please choose how to proceed." : "Filen skrevs över. Dina aktuella ändringar kan inte sparas automatiskt. Välj hur du vill fortsätta.",
    "The document could not be loaded. Please check your internet connection." : "Det gick inte att läsa in dokumentet. Kontrollera internetanslutningen.",
    "Reconnect" : "Återanslut",
    "You've been disconnected from the server." : "Du har blivit frånkopplad från servern.",
    "This file is opened read-only as it is currently locked by {user}." : "Den här filen öppnas skrivskyddad eftersom den för närvarande är låst av {user}.",
    "Insert below" : "Infoga nedanför",
    "Click for options, hold to drag" : "Klicka för alternativ, håll ned för att dra",
    "Failed to update the guest name." : "Det gick inte att uppdatera gästnamnet.",
    "Enter your name so other people can see who is editing" : "Ange ditt namn så att andra kan se vem som redigerar",
    "Enter your name" : "Ange ditt namn",
    "submit" : "skicka",
    "you" : "dig",
    "edit" : "redigera",
    "Uploading multiple attachments failed." : "Det gick inte att ladda upp flera bilagor.",
    "Uploading attachment failed: {error}" : "Det gick inte att ladda upp bilagan: {error}",
    "Uploading attachment failed." : "Det gick inte att ladda upp bilagan.",
    "Failed to insert from Files" : "Det gick inte att infoga från Filer",
    "Failed to create attachment" : "Det gick inte att skapa bilaga",
    "Offline since {time}." : "Offline sedan {time}.",
    "Not connected to the cloud." : "Inte ansluten till molnet.",
    "Preview options" : "Förhandsgranskning alternativ",
    "Text only" : "Endast text",
    "Show link preview" : "Visa förhandsgranskning för länk",
    "Open in new tab" : "Öppna i ny flik",
    "Copy link" : "Kopiera länk",
    "Remove link" : "Ta bort länk",
    "Active people" : "Aktiva användare",
    "Offline" : "Frånkopplad",
    "Offline, changes will be saved when online" : "Offline, ändringar kommer att sparas när du är online",
    "Saving …" : "Sparar …",
    "Saved" : "Sparad",
    "Last saved {lastSave}" : "Senast sparad {lastSave}",
    "The document has been changed outside of the editor. The changes cannot be applied." : "Dokumentet har ändrats utanför text-appen. Ändringarna kan inte tillämpas.",
    "Unsaved changes" : "Osparade ändringar",
    "Save document" : "Spara dokument",
    "Last saved" : "Senast sparad",
    "Close table of contents" : "Stäng innehållsförteckningen",
    "Pin table of contents" : "Fäst innehållsförteckning",
    "Table of contents" : "Innehållsförteckning",
    "Formatting and shortcuts" : "Formatering och genvägar",
    "Speed up your writing with simple shortcuts." : "Snabba upp ditt skrivande med enkla genvägar.",
    "Just type the Markdown syntax or use keyboard shortcuts from below." : "Skriv Markdown-syntaxen eller använd kortkommandon nedan.",
    "Just type the Markdown syntax from below." : "Skriv Markdown-syntaxen nedan.",
    "Style" : "Stil",
    "Syntax" : "Syntax",
    "Keyboard shortcuts" : "Kortkommandon",
    "New paragraph" : "Nytt stycke",
    "Enter" : "Ange",
    "Hard line break" : "Hård linjebrytning",
    "followed by" : "följd av",
    "Backspace" : "Backsteg",
    "Shift" : "Shift",
    "Bold" : "Fetstil",
    "Bold text" : "Fetstil",
    "Italic" : "Kursiv",
    "Italicized text" : "Kursiv stil",
    "Underline" : "Understrykning",
    "Underlined text" : "Understruken text",
    "Strikethrough" : "Genomstruken",
    "Mistaken text" : "Genomstruken text",
    "Highlight" : "Markera",
    "Highlighted text" : "Markerad text",
    "Heading 1" : "Rubrik 1",
    "Heading level 1" : "Rubrik nivå 1",
    "Heading 6" : "Rubrik 6",
    "Heading level 6" : "Rubrik nivå 6",
    "Unordered list" : "Punktlista",
    "An item" : "Ett objekt",
    "Ordered list" : "Numrerad lista",
    "First item" : "Första objektet",
    "Checklist" : "Checklista",
    "To-Do item" : "Att göra-objekt",
    "Blockquote" : "Citatblock",
    "Quoted text" : "Citerad text",
    "Some code" : "Någon kod",
    "Link" : "Länk",
    "Alt" : "Alt",
    "Emoji" : "Emoji",
    "emoji" : "emoji",
    "Mention someone" : "Nämn någon",
    "name" : "namn",
    "Smart picker" : "Smartväljare",
    "something" : "något",
    "Action" : "Åtgärd",
    "Undo" : "Ångra",
    "Redo" : "Gör om",
    "Toggle outline" : "Visa/dölj översikt",
    "Open link" : "Öppna länken",
    "No link available to open" : "Ingen länk tillgänglig att öppna",
    "Cannot open links with unsafe protocols" : "Kan inte öppna länkar med osäkra protokoll",
    "Edit link" : "Redigera länk",
    "Save changes" : "Spara ändringar",
    "URL" : "URL",
    "Changes" : "Ändringar",
    "Full documents" : "Hela dokument",
    "Markdown source" : "Markdown-källa",
    "Before" : "Före",
    "After" : "Efter",
    "Before document" : "Före dokumentet",
    "After document" : "Efter dokumentet",
    "Detailed rendered comparison unavailable" : "Detaljerad renderad jämförelse är inte tillgänglig",
    "Change {current} of {total}: {label}" : "Ändring {current} av {total}: {label}",
    "No rendered changes" : "Inga renderade ändringar",
    "Version comparison" : "Versionsjämförelse",
    "Comparison view" : "Jämförelsevy",
    "Change navigation" : "Ändringsnavigering",
    "Previous change" : "Föregående ändring",
    "Next change" : "Nästa ändring",
    "Hide formatting-only changes" : "Dölj ändringar som bara gäller formatering",
    "{count} hidden" : "{count} dolda",
    "Show" : "Visa",
    "All rendered changes are formatting-only and hidden by the current filter." : "Alla renderade ändringar gäller bara formatering och döljs av det aktuella filtret.",
    "No rendered differences — Markdown syntax differs." : "Inga renderade skillnader – Markdown-syntaxen skiljer sig.",
    "Open Markdown source" : "Öppna Markdown-källa",
    "No differences." : "Inga skillnader.",
    "Version to display" : "Version att visa",
    "Source compares literal Markdown, so its change groups can differ from rendered changes." : "Källan jämför ordagrann Markdown, så dess ändringsgrupper kan skilja sig från de renderade ändringarna.",
    "removed" : "borttagen",
    "{count} trailing spaces" : "{count} avslutande blanksteg",
    "No newline at end of file" : "Ingen nyrad i slutet av filen",
    "Markdown source comparison" : "Jämförelse av Markdown-källa",
    "Loading Markdown source differences…" : "Läser in skillnader i Markdown-källan…",
    "Could not load Markdown source differences." : "Det gick inte att läsa in skillnader i Markdown-källan.",
    "Retry" : "Försök igen",
    "Detailed source differences exceeded the safe processing limit." : "Detaljerade källskillnader överskred gränsen för säker bearbetning.",
    "Line endings changed: {before} → {after}" : "Radslut ändrades: {before} → {after}",
    "Source change {current} of {total}" : "Källändring {current} av {total}",
    "Source version to display" : "Källversion att visa",
    "Hide unchanged lines" : "Dölj oförändrade rader",
    "Show {count} unchanged lines" : "Visa {count} oförändrade rader",
    "Show more unchanged lines" : "Visa fler oförändrade rader",
    "Additional unchanged lines are hidden to keep the comparison responsive." : "Ytterligare oförändrade rader är dolda för att jämförelsen ska svara snabbt.",
    "Lines {before} / {after}" : "Rader {before} / {after}",
    "Complete Markdown source" : "Fullständig Markdown-källa",
    "Complete Markdown source is shown because a detailed comparison is unavailable." : "Den fullständiga Markdown-källan visas eftersom en detaljerad jämförelse inte är tillgänglig.",
    "Source preview was truncated to the display limit." : "Källförhandsvisningen kapades vid visningsgränsen.",
    "Edit block formula" : "Redigera blockformel",
    "Edit inline formula" : "Redigera inline-formel",
    "LaTeX formula" : "LaTeX-formel",
    "Disabled because you are currently offline." : "Inaktiverad eftersom du för närvarande är offline.",
    "Attachments cannot be created or uploaded because this file is shared from another cloud." : "Bilagor kan inte skapas eller laddas upp eftersom den här filen delas från ett annat moln.",
    "Upload from computer" : "Ladda upp från dator",
    "Insert from Files" : "Infoga från Filer",
    "Formatting help" : "Formateringshjälp",
    "Link to file or folder" : "Länk till fil eller mapp",
    "Link to website" : "Länk till webbplats",
    "Update link" : "Uppdatera länk",
    "Open the Smart Picker" : "Öppna Smartväljaren",
    "{menuItemName}, \"{selectedSubMenuItemName}\" is selected" : "{menuItemName}, ”{selectedSubMenuItemName}” är valt",
    "Insert result" : "Infoga resultat",
    "Insert" : "Infoga",
    "Nextcloud Assistant result copied" : "Nextcloud Assistant resultat har kopierats",
    "Could not copy result to clipboard" : "Det gick inte att kopiera resultat till urklipp",
    "Translate" : "Översätt",
    "Show assistant results" : "Visa assistent resultat",
    "Assistant results" : "Assistent resultat",
    "Nextcloud Assistant" : "Nextcloud Assistant",
    "Show result" : "Visa resultat",
    "Delete task" : "Ta bort uppgift",
    "_%n word_::_%n words_" : ["%n ord","%n ord"],
    "_%n char_::_%n chars_" : ["%n tecken","%n tecken"],
    "Remaining actions" : "Återstående åtgärder",
    "Editor actions" : "Redigeringsåtgärder",
    "Formatting menu bar" : "Formatera menyraden",
    "Full width editor" : "Full bredd editor",
    "No emoji found" : "Ingen emoji hittades",
    "No command found" : "Inget kommando hittades",
    "No user found" : "Ingen användare hittades",
    "No suggestion found" : "Inget förslag hittades",
    "Uploading attachments is disabled because the file is shared from another cloud." : "Uppladdning av bilagor är inaktiverad eftersom filen delas från ett annat moln.",
    "Upload" : "Ladda upp",
    "Insert Table" : "Infoga Tabell",
    "Smart Picker" : "Smartväljare",
    "Copy code block" : "Kopiera kodblock",
    "Copy code" : "Kopiera kod",
    "Code block options" : "Kodblocksalternativ",
    "Code block language" : "Språk för kodblock",
    "e.g. php, javascript, json…" : "t.ex. php, javascript, json…",
    "Source code" : "Källkod",
    "Diagram" : "Diagram",
    "Both" : "Både",
    "Mermaid documentation" : "Mermaid dokumentation",
    "Failed to load" : "Det gick inte att läsa in",
    "Could not find image in attachments list." : "Det gick inte att hitta bilden i listan med bilagor.",
    "Open attachment {name}" : "Öppna bilaga {name}",
    "Delete this attachment" : "Ta bort denna bilaga",
    "Open image {name}" : "Öppna bild {name}",
    "Delete this image" : "Ta bort denna bild",
    "Left align column" : "Vänsterjusterad kolumn",
    "Center align column" : "Centrerad kolumn",
    "Right align column" : "Högerjusterad kolumn",
    "Sort ascending" : "Sortera stigande",
    "Sort descending" : "Sortera fallande",
    "Add column before" : "Lägg till kolumn innan",
    "Add column after" : "Lägg till kolumn efter",
    "Delete this column" : "Ta bort denna kolumn",
    "Add row before" : "Lägg till rad innan",
    "Add row after" : "Lägg till rad efter",
    "Delete this row" : "Ta bort denna rad",
    "Delete this table" : "Ta bort denna tabell",
    "Add row below" : "Lägg till rad nedan",
    "Show folder description" : "Visa mappbeskrivning",
    "Image description changed" : "Bildbeskrivning ändrades",
    "Link target changed" : "Länkmål ändrades",
    "Link changed" : "Länk ändrades",
    "Link preview changed" : "Länkförhandsvisning ändrades",
    "Task state changed" : "Uppgiftsstatus ändrades",
    "Heading level changed" : "Rubriknivå ändrades",
    "List start changed" : "Listans start ändrades",
    "Code language changed" : "Kodspråk ändrades",
    "Text direction changed" : "Textriktning ändrades",
    "Table structure changed" : "Tabellstruktur ändrades",
    "Table alignment changed" : "Tabelljustering ändrades",
    "Callout type changed" : "Typ av informationsruta ändrades",
    "Details state changed" : "Detaljernas status ändrades",
    "Attribute changed" : "Attribut ändrades",
    "Inline code" : "Infogad kod",
    "{formatting} changed" : "{formatting} ändrades",
    "Hide outline" : "Göm översikt",
    "Show outline" : "Visa översikt",
    "Done" : "Klar",
    "Headings" : "Rubriker",
    "Heading 2" : "Rubrik 2",
    "Heading 3" : "Rubrik 3",
    "Heading 4" : "Rubrik 4",
    "Heading 5" : "Rubrik 5",
    "Lists" : "Listor",
    "To-Do list" : "Att göra-lista",
    "Increase indentation" : "Öka indraget",
    "Decrease indentation" : "Minska indraget",
    "Blocks" : "Block",
    "Inline math" : "Inline-matematik",
    "Block math" : "Blockmatematik",
    "Info callout" : "Informationsruta",
    "Success callout" : "Bekräftelseinfo",
    "Warning callout" : "Varningsinfo",
    "Danger callout" : "Kritisk varningsinfo",
    "Question callout" : "Frågeruta",
    "Annotations" : "Anteckningar",
    "Show annotations" : "Visa anteckningar",
    "Hide annotations" : "Dölj anteckningar",
    "Insert link" : "Infoga länk",
    "Insert attachment" : "Infoga bilaga",
    "Insert emoji" : "Infoga emoji",
    "Command" : "Kommando",
    "Ctrl" : "Ctrl",
    "Option" : "Alternativ",
    "Suggestions" : "Rekommendationer",
    "Formatting" : "Formatering",
    "Link a file" : "Länka en fil",
    "Start writing or type '/' to add…" : "Börja skriva eller skriv ’/’ för att lägga till…",
    "Select file or folder to link to" : "Välj fil eller mapp att länka till",
    "Choose" : "Välj",
    "Choose {file}" : "Välj {file}",
    "Add folder description" : "Lägg till mappbeskrivning",
    "\"{name}\" already exist!" : "\"{name}\" finns redan!",
    "Created \"{name}\"" : "Skapade \"{name}\"",
    "Copied to the clipboard" : "Kopierat till urklipp",
    "Could not copy to the clipboard" : "Det gick inte att kopiera till urklipp",
    "A table was pasted into a table. Nested tables are not supported." : "En tabell klistrades in i en tabell. Nästlade tabeller stöds inte",
    "Link to this section" : "Länk till detta avsnitt",
    "text document" : "textdokument",
    "New text file" : "Ny textfil",
    "Overwrite the file and save the current changes" : "Skriv över filen och spara de aktuella ändringarna",
    "Discard the current changes and load the latest version" : "Ignorera de aktuella ändringarna och läs in den senaste versionen",
    "Close outline view" : "Stäng översiktsvy",
    "Outline" : "Översikt",
    "Not connected. Cannot update guest name." : "Inte ansluten. Det går inte att uppdatera gästnamnet.",
    "Insert an attachment" : "Infoga en bilaga",
    "Saving …" : "Sparar …",
    "Translate from {language}" : "Översätt från {language}",
    "Translate to {language}" : "Översätt till {language}",
    "Translation failed" : "Det gick inte att översätta",
    "To translate individual parts of the text, select it before using the translate function." : "För att översätta enskilda delar av texten, välj den innan du använder översättningsfunktionen.",
    "Translate from" : "Översätt från",
    "Select language" : "Välj språk",
    "Text to translate from" : "Text att översätta från",
    "to" : "till",
    "Translate to" : "Översätt till",
    "Translated text result" : "Översatt textresultat",
    "Replace" : "Ersätt"
},
"nplurals=2; plural=(n != 1);");
