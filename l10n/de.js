OC.L10N.register(
    "text",
    {
    "The file was uploaded" : "Die Datei wurde hochgeladen",
    "The uploaded file exceeds the upload_max_filesize directive in php.ini" : "Die hochgeladene Datei übersteigt das Limit upload_max_filesize in der Datei php.ini",
    "The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form" : "Die hochgeladene Datei übersteigt das Limit MAX_FILE_SIZE im HTML-Formular",
    "The file was only partially uploaded" : "Die Datei wurde nur teilweise hochgeladen",
    "No file was uploaded" : "Keine Datei wurde hochgeladen",
    "Missing a temporary folder" : "Temporärer Ordner nicht vorhanden",
    "Could not write file to disk" : "Die Datei konnte nicht auf die Festplatte geschrieben werden",
    "A PHP extension stopped the file upload" : "Eine PHP-Erweiterung hat das Hochladen der Datei gestoppt",
    "No file uploaded or file size exceeds maximum of %s" : "Keine Datei hochgeladen oder die Dateigröße überschreitet %s",
    "File not found" : "Datei nicht gefunden",
    "Nextcloud Text" : "Nextcloud Text",
    "Text document" : "Textdokument",
    "Create new text file" : "Neue Textdatei erstellen",
    "Editing session has expired. Please reload the page." : "Die Bearbeitungssitzung ist abgelaufen. Bitte die Seite neu laden.",
    "{user} has mentioned you in the text document {node}" : "{user} hat dich in dem Textdokument {node} erwähnt.",
    "This file cannot be displayed as download is disabled by the share" : "Diese Datei kann nicht angezeigt werden, da das Herunterladen durch die Freigabe deaktiviert ist",
    "This document is read-only." : "Dieses Dokument ist schreibgeschützt",
    "Read-only permission cannot save document changes. Please reload the page." : "Mit der Berechtigung \"Nur Lesen\" können keine Änderungen am Dokument gespeichert werden. Bitte diese Seite erneut laden.",
    "Readme" : "Anleitung",
    "Text" : "Text",
    "📝 Collaborative document editing" : "📝 Gemeinschaftliche Dokumentenbearbeitung",
    "** 📝 Collaborative document editing!**\n\n- **📝 Focused writing:** No distractions, only the formatting you need.\n- **🙋 Work together:** Share and collaborate with friends and colleagues, no matter if they use Nextcloud or not!\n- **💾 Open format:** Files are saved as [Markdown](https://en.wikipedia.org/wiki/Markdown), so you can edit them from any other text app too.\n- **✊ Strong foundation:** We use [🐈 tiptap](https://tiptap.scrumpy.io) which is based on [🦉 ProseMirror](https://prosemirror.net) – huge thanks to them!" : "** 📝 Gemeinschaftliche Dokumentenbearbeitung!***\n\n- **📝 Fokussiertes Schreiben:** Keine Ablenkungen, nur die Formatierung, die du benötigst.\n- **🙋 Arbeite zusammen:** Teile und arbeite mit Freunden und Kollegen zusammen, egal ob du Nextcloud nutzt oder nicht!\n- **💾 Offenes Format:** Dateien werden als [Markdown](https://en.wikipedia.org/wiki/Markdown) gespeichert, so dass du die Dateien auch in jeder anderen Text-App bearbeiten kannst.\n- **✊ Starkes Fundament:** Wir verwenden[🐈 tiptap](https://tiptap.scrumpy.io), das auf[🦉 ProseMirror](https://prosemirror.net) basiert - vielen Dank an die Beiden!",
    "Your editing permissions have been revoked. The document is now read-only." : "Deine Bearbeitungsrechte wurden entzogen. Das Dokument ist nun schreibgeschützt.",
    "You now have edit permissions for this document." : "Du verfügst nun über Bearbeitungsrechte für dieses Dokument.",
    "Keep my local changes and overwrite the remote version" : "Meine lokalen Änderungen beibehalten und die Remote-Version überschreiben",
    "Discard my local changes and use the remote version" : "Meine lokalen Änderungen verwerfen und die Remote-Version verwenden",
    "Comments" : "Kommentare",
    "Previous comment" : "Vorheriger Kommentar",
    "Next comment" : "Nächster Kommentar",
    "Close" : "Schließen",
    "Guest" : "Gast",
    "guest" : "Gast",
    "Edit" : "Bearbeiten",
    "Delete" : "Löschen",
    "Save" : "Speichern",
    "Cancel" : "Abbrechen",
    "Enter your name to comment" : "Deinen Namen eingeben, um einen Kommentar zu hinterlassen",
    "Add a comment…" : "Einen Kommentar hinzufügen …",
    "Comment" : "Kommentar",
    "Reply" : "Antwort",
    "Front matter changed" : "Vorwort geändert",
    "Image changed" : "Bild geändert",
    "Mention changed" : "Erwähnung geändert",
    "Mathematics changed" : "Mathematik geändert",
    "Footnote changed" : "Fußnote geändert",
    "Divider changed" : "Teiler geändert",
    "Changed content" : "Geänderter Inhalt",
    "Paragraph" : "Absatz",
    "Heading" : "Überschrift",
    "List item" : "Listenelement",
    "Task" : "Aufgabe",
    "Table" : "Tabelle",
    "Table row" : "Tabellenzeile",
    "Table cell" : "Tabellenzelle",
    "Image" : "Bild",
    "Code block" : "Codeblock",
    "Quote" : "Zitat",
    "Callout" : "Aufruf",
    "Details" : "Details",
    "Footnote" : "Fußnote",
    "Footnote reference" : "Fußnotenreferenz",
    "Front matter" : "Titelei",
    "Mathematics" : "Mathematik",
    "Mention" : "Erwähnung",
    "Link preview" : "Link-Vorschau",
    "Added" : "Hinzugefügt",
    "Removed" : "Entfernt",
    "Changed" : "Geändert",
    "Moved" : "Verschoben",
    "Content added" : "Inhalt hinzugefügt",
    "Content removed" : "Inhalt entfernt",
    "Content changed" : "Inhalt geändert",
    "No content" : "Kein Inhalt",
    "Block-level" : "Blockebene",
    "Moved section" : "Bereich verschoben",
    "Structure changed" : "Struktur geändert",
    "{context} added" : "{context} hinzugefügt",
    "{context} removed" : "{context} entfernt",
    "{context} changed" : "{context} geändert",
    "Table column added" : "Tabellenspalte hinzugefügt",
    "Table column removed" : "Tabellenspalte entfernt",
    "Table column changed" : "Tabellenspalte geändert",
    "Section: {section}" : "Bereich: {section}",
    "Change pages" : "Seiten ändern",
    "Previous" : "Zurück",
    "Showing changes {from}–{to} of {total}" : "Änderungen anzeigen {from}–{to} von {total}",
    "Next" : "Weiter",
    "Document start" : "Dokumentenbeginn",
    "Block-level change; fine inline detail is unavailable" : "Änderung auf Blockebene; feine Inline-Details sind nicht verfügbar",
    "_%n change_::_%n changes_" : ["%n Änderung","%n Änderungen"],
    "_%n edit_::_%n edits_" : ["%n Bearbeitung","%n Bearbeitungen"],
    "Reload" : "Aktualisieren",
    "The file was overwritten. Your current changes cannot be auto-saved. Please choose how to proceed." : "Die Datei wurde überschrieben. Die aktuellen Änderungen können nicht automatisch gespeichert werden. Bitte auswählen, wie fortgefahren werden soll.",
    "The document could not be loaded. Please check your internet connection." : "Das Dokument konnte nicht geladen werden. Bitte die Internetverbindung überprüfen.",
    "Reconnect" : "Wiederverbinden",
    "You've been disconnected from the server." : "Die Verbindung zum Server wurde getrennt.",
    "This file is opened read-only as it is currently locked by {user}." : "Diese Datei ist schreibgeschützt geöffnet, da sie derzeit von {user} gesperrt ist.",
    "Insert below" : "Unten einfügen",
    "Click for options, hold to drag" : "Zum Anzeigen der Optionen klicken, zum Ziehen gedrückt halten",
    "Failed to update the guest name." : "Gastname konnte nicht aktualisiert werden.",
    "Enter your name so other people can see who is editing" : "Gib deinen Namen ein, so dass andere Benutzer sehen können, wer bearbeitet",
    "Enter your name" : "Gib deinen Namen ein",
    "submit" : "Übermitteln",
    "you" : "Du",
    "edit" : "Bearbeiten",
    "Uploading multiple attachments failed." : "Das Hochladen mehrerer Anhänge ist fehlgeschlagen.",
    "Uploading attachment failed: {error}" : "Hochladen des Anhangs fehlgeschlagen: {error}",
    "Uploading attachment failed." : "Das Hochladen des Anhangs ist fehlgeschlagen.",
    "Failed to insert from Files" : "Einfügen von Dateien fehlgeschlagen",
    "Failed to create attachment" : "Anhang konnte nicht erstellt werden",
    "Offline since {time}." : "Offline seit {time}.",
    "Not connected to the cloud." : "Nicht mit der Cloud verbunden.",
    "Preview options" : "Vorschauoptionen",
    "Text only" : "Nur Text",
    "Show link preview" : "Link-Vorschau anzeigen",
    "Open in new tab" : "In einem neuen Tab öffnen",
    "Copy link" : "Link kopieren",
    "Remove link" : "Link entfernen",
    "Active people" : "Aktive Benutzer",
    "Offline" : "Offline",
    "Offline, changes will be saved when online" : "Offline. Änderungen werden gespeichert, wenn du online bist.",
    "Saving …" : "Speichere …",
    "Saved" : "Gespeichert",
    "Last saved {lastSave}" : "Zuletzt gespeichert {lastSave}",
    "The document has been changed outside of the editor. The changes cannot be applied." : "Das Dokument wurde außerhalb des Editors gespeichert. Die Änderungen könnten nicht angewandt werden.",
    "Unsaved changes" : "Ungespeicherte Änderungen",
    "Save document" : "Dokument speichern",
    "Last saved" : "Zuletzt gespeichert",
    "Close table of contents" : "Inhaltsverzeichnis schließen",
    "Pin table of contents" : "Inhaltsverzeichnis anheften",
    "Table of contents" : "Inhaltsverzeichnis",
    "Formatting and shortcuts" : "Formatierung und Tastaturkürzel",
    "Speed up your writing with simple shortcuts." : "Beschleunige dein Schreiben mit einfachen Tastaturkürzeln",
    "Just type the Markdown syntax or use keyboard shortcuts from below." : "Gib die Markdown-Syntax ein oder verwende die untenstehenden Tastaturkürzel.",
    "Just type the Markdown syntax from below." : "Gib die untenstehende Markdown-Syntax ein.",
    "Style" : "Stil",
    "Syntax" : "Syntax",
    "Keyboard shortcuts" : "Tastaturkürzel",
    "New paragraph" : "Neuer Absatz",
    "Enter" : "Enter",
    "Hard line break" : "Harter Zeilenumbruch",
    "followed by" : "gefolgt von",
    "Backspace" : "Rücktaste",
    "Shift" : "Umschalttaste",
    "Bold" : "Fett",
    "Bold text" : "Fetter Text",
    "Italic" : "Kursiv",
    "Italicized text" : "Kursiver Text",
    "Underline" : "Unterstreichen",
    "Underlined text" : "Unterstrichener Text",
    "Strikethrough" : "Durchgestrichen",
    "Mistaken text" : "Falscher Text",
    "Highlight" : "Hervorheben",
    "Highlighted text" : "Hervorgehobener Text",
    "Heading 1" : "Überschrift 1",
    "Heading level 1" : "Überschriftsebene 1",
    "Heading 6" : "Überschrift 6",
    "Heading level 6" : "Überschriftsebene 6",
    "Unordered list" : "Ungeordnete Liste",
    "An item" : "ein Listenelement",
    "Ordered list" : "Geordnete Liste",
    "First item" : "erstes Listenelement",
    "Checklist" : "Checkliste",
    "To-Do item" : "Aufgaben-Element",
    "Blockquote" : "Blockzitat",
    "Quoted text" : "Zitat",
    "Some code" : "Code",
    "Link" : "Link",
    "Alt" : "Alt",
    "Emoji" : "Emoji",
    "emoji" : "Emoji",
    "Mention someone" : "Jemanden erwähnen",
    "name" : "Name",
    "Smart picker" : "Intelligente Auswahl",
    "something" : "etwas",
    "Action" : "Aktion",
    "Undo" : "Rückgängig",
    "Redo" : "Wiederherstellen",
    "Toggle outline" : "Gliederung umschalten",
    "Open link" : "Link öffnen",
    "No link available to open" : "Kein Link zum Öffnen verfügbar",
    "Cannot open links with unsafe protocols" : "Links mit unsicheren Protokollen können nicht geöffnet werden",
    "Edit link" : "Link bearbeiten",
    "Save changes" : "Änderungen speichern",
    "URL" : "URL",
    "Changes" : "Änderungen",
    "Full documents" : "Ganze Dokumente",
    "Markdown source" : "Markdown-Quelltext",
    "Before" : "Vor",
    "After" : "Nach",
    "Before document" : "Vor dem Dokument",
    "After document" : "Nach dem Dokument",
    "Detailed rendered comparison unavailable" : "Detaillierter Rendering-Vergleich nicht verfügbar",
    "Change {current} of {total}: {label}" : "{current} von {total} ändern: {label}",
    "No rendered changes" : "Keine Änderungen gerendert",
    "Version comparison" : "Versionsvergleich",
    "Comparison view" : "Vergleichsansicht",
    "Change navigation" : "Navigation ändern",
    "Previous change" : "Vorhergehende Änderungen",
    "Next change" : "Nächste Änderung",
    "Hide formatting-only changes" : "Formatierungsänderungen ausblenden",
    "{count} hidden" : "{count} versteckt",
    "Show" : "Anzeigen",
    "All rendered changes are formatting-only and hidden by the current filter." : "Alle gerenderten Änderungen sind rein formatierend und werden vom aktuellen Filter ausgeblendet.",
    "No rendered differences — Markdown syntax differs." : "Keine Unterschiede in der Darstellung – die Markdown-Syntax ist unterschiedlich.",
    "Open Markdown source" : "Markdown-Quellcode öffnen",
    "No differences." : "Keine Unterschiede.",
    "Version to display" : "Anzuzeigende Version",
    "Source compares literal Markdown, so its change groups can differ from rendered changes." : "Source vergleicht wörtliches Markdown, daher können sich die Änderungsgruppen von den gerenderten Änderungen unterscheiden.",
    "removed" : "Entfernt",
    "{count} trailing spaces" : "{count} nachgestellte Leerzeichen",
    "No newline at end of file" : "Am Ende der Datei fehlt eine neue Zeile",
    "Markdown source comparison" : "Markdown-Quellenvergleich",
    "Loading Markdown source differences…" : "Markdown-Quellunterschiede werden geladen …",
    "Could not load Markdown source differences." : "Die Unterschiede im Markdown-Quellcode konnten nicht geladen werden.",
    "Retry" : "Erneut versuchen",
    "Detailed source differences exceeded the safe processing limit." : "Die detaillierten Quellunterschiede überschritten die zulässige Verarbeitungsgrenze.",
    "Line endings changed: {before} → {after}" : "Zeilenumbrüche geändert: {before} → {after}",
    "Source change {current} of {total}" : "Quellenwechsel {current} von {total}",
    "Source version to display" : "Anzuzeigende Quellversion",
    "Hide unchanged lines" : "Unveränderte Zeilen ausblenden",
    "Show {count} unchanged lines" : "{count} unveränderte Zeilen anzeigen",
    "Show more unchanged lines" : "Mehr unveränderte Zeilen anzeigen",
    "Additional unchanged lines are hidden to keep the comparison responsive." : "Um die Vergleichsdarstellung reaktionsschnell zu halten, werden zusätzliche, unveränderte Zeilen ausgeblendet.",
    "Lines {before} / {after}" : "Zeilen {before} / {after}",
    "Complete Markdown source" : "Vollständiger Markdown-Quelltext",
    "Complete Markdown source is shown because a detailed comparison is unavailable." : "Der vollständige Markdown-Quelltext wird angezeigt, da ein detaillierter Vergleich nicht verfügbar ist.",
    "Source preview was truncated to the display limit." : "Die Quellvorschau wurde auf die Anzeigegrenze gekürzt.",
    "Edit block formula" : "Block-Formel bearbeiten",
    "Edit inline formula" : "Inline-Formel bearbeiten",
    "LaTeX formula" : "LaTeX-Formel",
    "Disabled because you are currently offline." : "Deaktiviert, da du aktuell offline bist.",
    "Attachments cannot be created or uploaded because this file is shared from another cloud." : "Anhänge können nicht erstellt oder hochgeladen werden, da diese Datei aus einer anderen Cloud freigegeben wurde.",
    "Upload from computer" : "Vom Computer hochladen",
    "Insert from Files" : "Aus Dateien einfügen",
    "Formatting help" : "Formatierungshilfe",
    "Link to file or folder" : "Link zu Datei oder Ordner",
    "Link to website" : "Link zu Website",
    "Update link" : "Link aktualisieren",
    "Open the Smart Picker" : "Smart-Picker öffnen",
    "{menuItemName}, \"{selectedSubMenuItemName}\" is selected" : "\"{menuItemName}, \"{selectedSubMenuItemName}\" ist ausgewählt",
    "Insert result" : "Ergebnis einfügen",
    "Insert" : "Einfügen",
    "Nextcloud Assistant result copied" : "Ergebnis des Nextcloud Assistant kopiert",
    "Could not copy result to clipboard" : "Ergebnis konnte nicht in die Zwischenablage kopiert werden.",
    "Translate" : "Übersetzen",
    "Show assistant results" : "Ergebnisse des Assistenten anzeigen",
    "Assistant results" : "Assistentenergebnisse",
    "Nextcloud Assistant" : "Nextcloud Assistant",
    "Show result" : "Ergebnis anzeigen",
    "Delete task" : "Aufgabe löschen",
    "_%n word_::_%n words_" : ["%n Wort","%n Wörter"],
    "_%n char_::_%n chars_" : ["%n Zeichen","%n Zeichen"],
    "Remaining actions" : "Verbleibende Aktionen",
    "Editor actions" : "Editor-Aktionen",
    "Formatting menu bar" : "Formatierungsmenüleiste",
    "Full width editor" : "Editor in voller Breite",
    "No emoji found" : "Kein Emoji gefunden",
    "No command found" : "Kein Befehl gefunden",
    "No user found" : "Kein Benutzer gefunden",
    "No suggestion found" : "Kein Vorschlag gefunden",
    "Uploading attachments is disabled because the file is shared from another cloud." : "Das Hochladen von Anhängen ist deaktiviert, da die Datei aus einer anderen Cloud freigegeben wurde.",
    "Upload" : "Hochladen",
    "Insert Table" : "Tabelle einfügen",
    "Smart Picker" : "Smart Picker",
    "Copy code block" : "Codeblock kopieren",
    "Copy code" : "Code kopieren",
    "Code block options" : "Codeblock-Optionen",
    "Code block language" : "Codeblock-Sprache",
    "e.g. php, javascript, json…" : "Z. B. php, javascript, json…",
    "Source code" : "Quelltext",
    "Diagram" : "Diagramm",
    "Both" : "Beide",
    "Mermaid documentation" : "Mermaid-Dokumentation",
    "Failed to load" : "Laden fehlgeschlagen",
    "Could not find image in attachments list." : "Bild konnte in der Anhangsliste nicht gefunden werden.",
    "Open attachment {name}" : "Anhang {name} öffnen",
    "Delete this attachment" : "Anhang löschen",
    "Open image {name}" : "Bild {name} öffnen",
    "Delete this image" : "Bild löschen",
    "Left align column" : "Spalte links ausrichten",
    "Center align column" : "Spalte zentriert ausrichten",
    "Right align column" : "Spalte rechts ausrichten",
    "Sort ascending" : "Aufsteigend sortieren",
    "Sort descending" : "Absteigend sortieren",
    "Add column before" : "Spalte davor hinzufügen",
    "Add column after" : "Spalte danach hinzufügen",
    "Delete this column" : "Diese Spalte löschen",
    "Add row before" : "Zeile davor hinzufügen",
    "Add row after" : "Zeile danach hinzufügen",
    "Delete this row" : "Diese Zeile löschen",
    "Delete this table" : "Diese Tabelle löschen",
    "Add row below" : "Zeile darunter hinzufügen",
    "Show folder description" : "Ordnerbeschreibung anzeigen",
    "Image description changed" : "Bildbeschreibung geändert",
    "Link target changed" : "Linkziel geändert",
    "Link changed" : "Link geändert",
    "Link preview changed" : "Link-Vorschau geändert",
    "Task state changed" : "Aufgabenstatus geändert",
    "Heading level changed" : "Überschriftenebene geändert",
    "List start changed" : "Listenstart geändert",
    "Code language changed" : "Codesprache geändert",
    "Text direction changed" : "Textrichtung geändert",
    "Table structure changed" : "Tabellenstruktur geändert",
    "Table alignment changed" : "Tabellenausrichtung geändert",
    "Callout type changed" : "Ausgabe-Typ geändert",
    "Details state changed" : "Details-Status geändert",
    "Attribute changed" : "Attribute geändert",
    "Inline code" : "Inline-Code",
    "{formatting} changed" : "{formatting} geändert",
    "Hide outline" : "Gliederung ausblenden",
    "Show outline" : "Gliederung anzeigen",
    "Done" : "Erledigt",
    "Headings" : "Überschriften",
    "Heading 2" : "Überschrift 2",
    "Heading 3" : "Überschrift 3",
    "Heading 4" : "Überschrift 4",
    "Heading 5" : "Überschrift 5",
    "Lists" : "Listen",
    "To-Do list" : "Aufgabenliste",
    "Increase indentation" : "Einrückung vergrößern",
    "Decrease indentation" : "Einrückung verkleinern",
    "Blocks" : "Blöcke",
    "Inline math" : "Inline-Mathematik",
    "Block math" : "Block-Mathematik",
    "Info callout" : "Info-Textbox",
    "Success callout" : "Erfolg-Textbox",
    "Warning callout" : "Warnung-Textbox",
    "Danger callout" : "Gefahr-Textbox",
    "Question callout" : "Frageaufruf",
    "Annotations" : "Notizen",
    "Show annotations" : "Notizen zeigen",
    "Hide annotations" : "Notizen ausblenden",
    "Insert link" : "Link einfügen",
    "Insert attachment" : "Anhang einfügen",
    "Insert emoji" : "Emoji einfügen",
    "Command" : "Befehl",
    "Ctrl" : "Strg",
    "Option" : "Option",
    "Suggestions" : "Vorschläge",
    "Formatting" : "Formatierung",
    "Link a file" : "Datei verlinken",
    "Start writing or type '/' to add…" : "Beginne mit dem Schreiben oder gebe '/' zum Hinzufügen ein…",
    "Select file or folder to link to" : "Datei oder Ordner zum Verknüpfen auswählen",
    "Choose" : "Auswählen",
    "Choose {file}" : "{file} auswählen",
    "Add folder description" : "Ordnerbeschreibung hinzufügen",
    "\"{name}\" already exist!" : "{name} existiert bereits!",
    "Created \"{name}\"" : "\"{name}\" erstellt",
    "Copied to the clipboard" : "In die Zwischenablage kopiert",
    "Could not copy to the clipboard" : "Konnte nicht in die Zwischenablage kopieren",
    "A table was pasted into a table. Nested tables are not supported." : "Eine Tabelle wurde in eine Tabelle eingefügt. Verschachtelte Tabellen werden nicht unterstützt.",
    "Link to this section" : "Verweis zu diesem Abschnitt",
    "text document" : "Textdokument",
    "New text file" : "Neue Textdatei",
    "Overwrite the file and save the current changes" : "Datei überschreiben und die aktuellen Änderungen speichern",
    "Discard the current changes and load the latest version" : "Aktuelle Änderungen verwerfen und die letzte Version laden",
    "Close outline view" : "Gliederungsansicht schließen",
    "Outline" : "Gliederung",
    "Not connected. Cannot update guest name." : "Nicht verbunden. Gastname kann nicht aktualisiert werden.",
    "Insert an attachment" : "Anhang einfügen",
    "Saving …" : "Speichern …",
    "Translate from {language}" : "Übersetzen von {language}",
    "Translate to {language}" : "Übersetzen nach {language}",
    "Translation failed" : "Übersetzung fehlgeschlagen",
    "To translate individual parts of the text, select it before using the translate function." : "Um einzelne Textteile zu übersetzen, wähle diese aus, bevor du die Übersetzungsfunktion verwendest.",
    "Translate from" : "Übersetzen von",
    "Select language" : "Sprache wählen",
    "Text to translate from" : "Text zu übersetzen von",
    "to" : "bis",
    "Translate to" : "Übersetzen in",
    "Translated text result" : "Ergebnis des übersetzten Textes",
    "Replace" : "Ersetzen"
},
"nplurals=2; plural=(n != 1);");
