OC.L10N.register(
    "text",
    {
    "The file was uploaded" : "Tiedosto lähetettiin",
    "The uploaded file exceeds the upload_max_filesize directive in php.ini" : "Ladattu tiedosto on suurempi kuin php.ini-tiedostossa määritelty upload_max_filesize",
    "The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form" : "Ladattu tiedosto on suurempi kuin HTML-lomakkeessa määritelty MAX_FILE_SIZE",
    "The file was only partially uploaded" : "Tiedosto lähetettiin vain osittain",
    "No file was uploaded" : "Tiedostoa ei lähetetty",
    "Missing a temporary folder" : "Väliaikaiskansio puuttuu",
    "Could not write file to disk" : "Ei voitu kirjoittaa levylle",
    "A PHP extension stopped the file upload" : "PHP-laajennus pysäytti tiedoston lähetyksen",
    "No file uploaded or file size exceeds maximum of %s" : "Tiedostoa ei lähetetty tai sen koko ylitti maksimin %s",
    "File not found" : "Tiedostoa ei löytynyt",
    "Nextcloud Text" : "Nextcloud Text",
    "Text document" : "Tekstiasiakirja",
    "Create new text file" : "Luo uusi tekstitiedosto",
    "Editing session has expired. Please reload the page." : "Muokkausistunto vanheni. Lataa sivu uudelleen.",
    "{user} has mentioned you in the text document {node}" : "{user} mainitsi sinut tekstiasiakirjassa {node}",
    "This document is read-only." : "Tämä asiakirja on kirjoitussuojattu.",
    "Readme" : "Lueminut",
    "Text" : "Teksti",
    "📝 Collaborative document editing" : "📝 Yhteistyöhön perustuvaa asiakirjojen muokkausta",
    "You now have edit permissions for this document." : "Sinulla on nyt muokkausoikeudet tähän asiakirjaan.",
    "Keep my local changes and overwrite the remote version" : "Säilytä paikalliset muutokset ja ylikirjoita etäversio",
    "Discard my local changes and use the remote version" : "Hylkää paikalliset muutokset ja käytä etäversiota",
    "Comments" : "Kommentit",
    "Previous comment" : "Edellinen kommentti",
    "Next comment" : "Seuraava kommentti",
    "Close" : "Sulje",
    "Guest" : "Vieras",
    "guest" : "vieras",
    "Edit" : "Muokkaa",
    "Delete" : "Delete",
    "Save" : "Tallenna",
    "Cancel" : "Peruuta",
    "Enter your name to comment" : "Kirjoita nimesi kommentoidaksesi",
    "Add a comment…" : "Lisää kommentti…",
    "Comment" : "Kommentti",
    "Reply" : "Vastaa",
    "Paragraph" : "Kappale",
    "Heading" : "Otsikko",
    "Task" : "Tehtävä",
    "Table" : "Taulukko",
    "Image" : "Kuva",
    "Code block" : "Koodilohko",
    "Quote" : "Lainaus",
    "Details" : "Tiedot",
    "Added" : "Lisätty",
    "Removed" : "Poistettu",
    "Changed" : "Muutettu",
    "No content" : "Ei sisältöä",
    "{context} added" : "{context} lisätty",
    "{context} removed" : "{context} poistettu",
    "{context} changed" : "{context} muutettu",
    "Previous" : "Edellinen",
    "Showing changes {from}–{to} of {total}" : "Näytetään muutokset {from}–{to} / {total}",
    "Next" : "Seuraava",
    "Reload" : "Päivitä",
    "Reconnect" : "Yhdistä uudelleen",
    "This file is opened read-only as it is currently locked by {user}." : "Tämä tiedosto on avattu vain luku -tilassa, koska se on tällä hetkellä lukittu käyttäjän {user} toimesta.",
    "Click for options, hold to drag" : "Paina kerran asetukset auki, paina pohjassa vetämistä varten",
    "Enter your name so other people can see who is editing" : "Kirjoita nimesi, jotta muut näkevät kuka muokkaa asiakirjaa",
    "Enter your name" : "Kirjoita nimesi",
    "you" : "sinä",
    "edit" : "muokkaa",
    "Show link preview" : "Näytä linkin esikatselu",
    "Open in new tab" : "Avaa uudessa välilehdessä",
    "Copy link" : "Kopioi linkki",
    "Remove link" : "Poista linkki",
    "Active people" : "Aktiiviset ihmiset",
    "Offline" : "Poissa",
    "Offline, changes will be saved when online" : "Ei yhteyttä. Muutokset tallennetaan, kun yhteys palautuu",
    "Saving …" : "Tallennetaan …",
    "Saved" : "Tallennettu",
    "Last saved {lastSave}" : "Viimeksi tallennettu {lastSave}",
    "The document has been changed outside of the editor. The changes cannot be applied." : "Dokumenttia on muutettu editorin ulkopuolella. Muutoksia ei voida tallentaa.",
    "Unsaved changes" : "Tallentamattomia muutoksia",
    "Save document" : "Tallenna asiakirja",
    "Last saved" : "Viimeksi tallennettu",
    "Close table of contents" : "Sulje sisällysluettelo",
    "Pin table of contents" : "Kiinnitä sisällysluettelo",
    "Table of contents" : "Sisällysluettelo",
    "Formatting and shortcuts" : "Muotoilu ja pikanäppäimet",
    "Speed up your writing with simple shortcuts." : "Nopeuta kirjoittamista pikanäppäimillä.",
    "Just type the Markdown syntax or use keyboard shortcuts from below." : "Kirjoita Markdown-syntaksia tai käytä alla olevia pikanäppäimiä.",
    "Just type the Markdown syntax from below." : "Kirjoita Markdown-syntaksia alapuolelta.",
    "Style" : "Tyyli",
    "Syntax" : "Syntaksi",
    "Keyboard shortcuts" : "Pikanäppäimet",
    "New paragraph" : "Uusi kappale",
    "Shift" : "Siirrä",
    "Bold" : "Lihavoitu",
    "Bold text" : "Lihavoitu teksti",
    "Italic" : "Kursivoitu",
    "Italicized text" : "Kursivoitu teksti",
    "Underline" : "Alleviivattu",
    "Underlined text" : "Alleviivattu teksti",
    "Strikethrough" : "Yliviivattu",
    "Heading 1" : "Otsikko 1",
    "Heading level 1" : "Otsikkotaso 1",
    "Heading 6" : "Otsikko 6",
    "Heading level 6" : "Otsikkotaso 6",
    "Unordered list" : "Lista",
    "An item" : "Tietue",
    "Ordered list" : "Numeroitu lista",
    "First item" : "Ensimmäinen tietue",
    "Checklist" : "Tarkistuslista",
    "Blockquote" : "Lohkolainaus",
    "Quoted text" : "Lainattu teksti",
    "Some code" : "Koodia",
    "Link" : "Linkki",
    "Emoji" : "Hymiö",
    "Mention someone" : "Mainitse käyttäjä",
    "name" : "nimi",
    "Smart picker" : "Älykäs valitsin",
    "Action" : "Toiminto",
    "Undo" : "Kumoa",
    "Redo" : "Tee uudelleen",
    "Open link" : "Avaa linkki",
    "Edit link" : "Muokkaa linkkiä",
    "Save changes" : "Tallenna muutokset",
    "URL" : "URL-osoite",
    "Changes" : "Muutokset",
    "Before" : "Ennen",
    "After" : "Jälkeen",
    "Show" : "Näytä",
    "No differences." : "Ei eroavuuksia.",
    "Retry" : "Yritä uudelleen",
    "LaTeX formula" : "LaTeX-kaava",
    "Upload from computer" : "Lähetä tietokoneelta",
    "Insert from Files" : "Syötä Tiedostot-sovelluksesta",
    "Formatting help" : "Muotoiluohje",
    "Link to file or folder" : "Linkki tiedostoon tai kansioon",
    "Link to website" : "Linkki verkkosivulle",
    "Update link" : "Päivitä linkki",
    "Open the Smart Picker" : "Avaa älykäs valitsin",
    "{menuItemName}, \"{selectedSubMenuItemName}\" is selected" : "{menuItemName}, \"{selectedSubMenuItemName}\" on valittu",
    "Insert result" : "Syötä tulos",
    "Insert" : "Lisää",
    "Nextcloud Assistant result copied" : "Nextcloud-avustajan tulos kopioitu",
    "Could not copy result to clipboard" : "Tulosta ei voitu kopioida leikepöydälle",
    "Translate" : "Käännä",
    "Show assistant results" : "Näytä avustajan tulokset",
    "Nextcloud Assistant" : "Nextcloud-avustaja",
    "Show result" : "Näytä tulos",
    "Delete task" : "Poista tehtävä",
    "_%n word_::_%n words_" : ["%n sana","%n sanaa"],
    "_%n char_::_%n chars_" : ["%n merkki","%n merkkiä"],
    "Formatting menu bar" : "Muotoiluvalikko",
    "No emoji found" : "Emojia ei löydy",
    "No command found" : "Komentoa ei löytynyt",
    "No user found" : "Käyttäjää ei löytynyt",
    "No suggestion found" : "Ehdotusta ei löytynyt",
    "Upload" : "Lähetä",
    "Insert Table" : "Lisää taulukko",
    "Smart Picker" : "Älykäs valitsin",
    "Copy code block" : "Kopioi koodilohko",
    "Copy code" : "Kopioi koodi",
    "Code block options" : "Koodilohkon asetukset",
    "Code block language" : "Koodilohkon kieli",
    "e.g. php, javascript, json…" : "esim. php, javascript, json…",
    "Source code" : "Lähdekoodi",
    "Diagram" : "Kaavio",
    "Both" : "Molemmat",
    "Mermaid documentation" : "Mermaid-dokumentaatio",
    "Failed to load" : "Lataaminen epäonnistui",
    "Open attachment {name}" : "Avaa liite {name}",
    "Delete this attachment" : "Poista tämä liite",
    "Open image {name}" : "Avaa kuva {name}",
    "Delete this image" : "Poista tämä kuva",
    "Add column before" : "Lisää sarake ennen",
    "Add column after" : "Lisää sarake jälkeen",
    "Delete this column" : "Poista tämä sarake",
    "Add row before" : "Lisää rivi ennen",
    "Add row after" : "Lisää rivi jälkeen",
    "Delete this row" : "Poista tämä rivi",
    "Delete this table" : "Poista tämä taulu",
    "Add row below" : "Lisää rivi alapuolelle",
    "Show folder description" : "Näytä kansion kuvaus",
    "{formatting} changed" : "{formatting} muutettu",
    "Done" : "Valmis",
    "Headings" : "Otsikot",
    "Heading 2" : "Otsikko 2",
    "Heading 3" : "Otsikko 3",
    "Heading 4" : "Otsikko 4",
    "Heading 5" : "Otsikko 5",
    "Lists" : "Luettelot",
    "To-Do list" : "Tehtävälista",
    "Increase indentation" : "Lisää sisennystä",
    "Decrease indentation" : "Vähennä sisennystä",
    "Blocks" : "Lohkot",
    "Mermaid diagram" : "Mermaid-kaavio",
    "Info callout" : "Tietolaatikko",
    "Success callout" : "Onnistumislaatikko",
    "Warning callout" : "Varoituslaatikko",
    "Danger callout" : "Vaara! -laatikko",
    "Insert link" : "Lisää linkki",
    "Insert attachment" : "Lisää liite",
    "Insert emoji" : "Lisää emoji",
    "Command" : "Komento",
    "Option" : "Valinta",
    "Suggestions" : "Ehdotukset",
    "Formatting" : "Muotoilu",
    "Start writing or type '/' to add…" : "Aloita kirjoittaminen tai kirjoita '/' lisätäksesi...",
    "Select file or folder to link to" : "Valitse tiedosto tai kansio, johon linkitetään",
    "Choose" : "Valitse",
    "Choose {file}" : "Valitse {file}",
    "Add folder description" : "Lisään kansion kuvaus",
    "\"{name}\" already exist!" : "\"{name}\" on jo olemassa!",
    "Created \"{name}\"" : "Luotu \"{name}\"",
    "Copied to the clipboard" : "Kopioitu leikepöydälle",
    "Could not copy to the clipboard" : "Ei voitu kopioida leikepöydälle",
    "Link to this section" : "Linkki tähän osioon",
    "text document" : "Tekstiasiakirja",
    "New text file" : "Uusi tekstitiedosto",
    "Insert an attachment" : "Lisää liite",
    "Saving …" : "Tallennetaan...",
    "Translate from {language}" : "Käännä kielestä {language}",
    "Translate to {language}" : "Käännä kielelle {language}",
    "Translation failed" : "Kääntäminen epäonnistui",
    "To translate individual parts of the text, select it before using the translate function." : "Kääntääksesi yksittäisen osan tekstiä, valitse haluamasi tekstiosa ennen käännöstoiminnon käyttöä.",
    "Translate from" : "Lähdekieli",
    "Select language" : "Valitse kieli",
    "to" : "päättyen",
    "Translate to" : "Kohdekieli",
    "Replace" : "Korvaa"
},
"nplurals=2; plural=(n != 1);");
