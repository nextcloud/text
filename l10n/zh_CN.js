OC.L10N.register(
    "text",
    {
    "The file was uploaded" : "文件已上传",
    "The uploaded file exceeds the upload_max_filesize directive in php.ini" : "上传文件超出了php.ini中upload_max_filesize的限制",
    "The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form" : "上传文件超出了HTML表单中MAX_FILE_SIZE的限制",
    "The file was only partially uploaded" : "仅部分文件成功上传",
    "No file was uploaded" : "未有上传的文件",
    "Missing a temporary folder" : "缺失一个临时文件夹",
    "Could not write file to disk" : "无法向磁盘中写入文件",
    "A PHP extension stopped the file upload" : "一个PHP扩展插件终止了文件上传",
    "No file uploaded or file size exceeds maximum of %s" : "没有文件上传成功或者文件大小超出了最大%s的限制",
    "File not found" : "文件未找到",
    "Nextcloud Text" : "Nextcloud 文本",
    "Text document" : "文本文档",
    "Create new text file" : "创建新的记事本文件",
    "Editing session has expired. Please reload the page." : "编辑会话超时。请重载页面。",
    "{user} has mentioned you in the text document {node}" : "{user} 在记事本文件 {node} 中提到你",
    "This file cannot be displayed as download is disabled by the share" : "由于分享时下载已被禁用，该文件无法显示。",
    "This document is read-only." : "此文档是只读的。",
    "Read-only permission cannot save document changes. Please reload the page." : "只读权限无法保存文档更改。请重新加载页面。",
    "Readme" : "自述文件",
    "Text" : "文本 ",
    "📝 Collaborative document editing" : "文档协作编辑",
    "** 📝 Collaborative document editing!**\n\n- **📝 Focused writing:** No distractions, only the formatting you need.\n- **🙋 Work together:** Share and collaborate with friends and colleagues, no matter if they use Nextcloud or not!\n- **💾 Open format:** Files are saved as [Markdown](https://en.wikipedia.org/wiki/Markdown), so you can edit them from any other text app too.\n- **✊ Strong foundation:** We use [🐈 tiptap](https://tiptap.scrumpy.io) which is based on [🦉 ProseMirror](https://prosemirror.net) – huge thanks to them!" : "** 📝 协作式的文档编辑！**\n\n- **📝 专注的写作：** 无干扰，只有您需要的格式化。\n- **🙋 一起工作：** 与朋友和同事共享协作，无论他们是否使用Nextcloud！\n- **💾 开放标准：** 文件将保存为 [Markdown](https://en.wikipedia.org/wiki/Markdown)，所以您也可以使用任何其他文本应用。\n- **✊ 雄厚基础：** 我们使用 [🐈 tiptap](https://tiptap.scrumpy.io) ，它基于 [🦉 ProseMirror](https://prosemirror.net) – 非常感谢它们！",
    "Your editing permissions have been revoked. The document is now read-only." : "您的编辑权限已被撤销。该文档现在是只读的。",
    "You now have edit permissions for this document." : "您现在拥有此文档的编辑权限。",
    "Keep my local changes and overwrite the remote version" : "保留我的本地更改并覆盖远程版本",
    "Discard my local changes and use the remote version" : "放弃我的本地更改并使用远程版本",
    "Comments" : "评论",
    "Previous comment" : "上一条评论",
    "Next comment" : "下一条评论",
    "Close" : "关闭",
    "Guest" : "访客",
    "guest" : "访客",
    "Edit" : "编辑",
    "Delete" : "删除",
    "Save" : "保存",
    "Cancel" : "取消",
    "Enter your name to comment" : "输入您的名称以发表评论",
    "Add a comment…" : "添加评论…",
    "Comment" : "评论",
    "Reply" : "回复",
    "Reload" : "重新加载",
    "The file was overwritten. Your current changes cannot be auto-saved. Please choose how to proceed." : "文件已被覆盖。无法自动保存您当前的更改。请选择如何继续。",
    "The document could not be loaded. Please check your internet connection." : "无法加载文档。请检查您的互联网连接。",
    "Reconnect" : "重新连接",
    "You've been disconnected from the server." : "您已与服务器断开连接。",
    "This file is opened read-only as it is currently locked by {user}." : "该文件以仅读模式打开，其正被用户 {user} 锁定。",
    "Insert below" : "下方插入",
    "Click for options, hold to drag" : "点击以查看选项，按住可拖动",
    "Failed to update the guest name." : "无法更新访客名称。",
    "Enter your name so other people can see who is editing" : "输入你的名称来让其他用户可以看到是谁在编辑。",
    "Enter your name" : "输入名称",
    "submit" : "提交",
    "you" : "您",
    "edit" : "编辑",
    "Uploading multiple attachments failed." : "上传多个附件失败。",
    "Uploading attachment failed: {error}" : "上传附件失败：{error}",
    "Uploading attachment failed." : "上传附件失败。",
    "Failed to insert from Files" : "无法从文件插入",
    "Failed to create attachment" : "无法创建附件",
    "Offline since {time}." : "自 {time} 起离线。",
    "Not connected to the cloud." : "未连接到云端。",
    "Preview options" : "预览选项",
    "Text only" : "仅文字",
    "Show link preview" : "显示链接预览",
    "Open in new tab" : "在新标签页中打开",
    "Copy link" : "复制链接",
    "Remove link" : "删除链接",
    "Active people" : "活动用户",
    "Offline" : "离线",
    "Offline, changes will be saved when online" : "离线，更改将会在上线后保存",
    "Saving …" : "正在保存 …",
    "Saved" : "已保存",
    "Last saved {lastSave}" : "最近保存 {lastSave}",
    "The document has been changed outside of the editor. The changes cannot be applied." : "本文档在编辑器外被更改。无法应用更改。",
    "Unsaved changes" : "未保存的更改",
    "Save document" : "保存文档",
    "Last saved" : "最近保存",
    "Close table of contents" : "关闭目录",
    "Pin table of contents" : "固定目录",
    "Table of contents" : "目录",
    "Formatting and shortcuts" : "排版和捷径",
    "Speed up your writing with simple shortcuts." : "利用简单快捷键加速写作",
    "Just type the Markdown syntax or use keyboard shortcuts from below." : "只需输入 markdown 语法或从下面使用键盘快捷键。",
    "Just type the Markdown syntax from below." : "只需从下面输入 markdown 语法",
    "Style" : "样式",
    "Syntax" : "语法",
    "Keyboard shortcuts" : "键盘快捷键",
    "New paragraph" : "新的段落",
    "Enter" : "输入",
    "Hard line break" : "强制换行",
    "followed by" : "后接",
    "Backspace" : "退格",
    "Shift" : "Shift",
    "Bold" : "加粗",
    "Bold text" : "粗文本",
    "Italic" : "斜体",
    "Italicized text" : "斜文本",
    "Underline" : "下划线",
    "Underlined text" : "下划线",
    "Strikethrough" : "删除线",
    "Mistaken text" : "错误文本",
    "Highlight" : "高亮",
    "Highlighted text" : "高亮文本",
    "Heading 1" : "标题 1",
    "Heading level 1" : "标题级 1",
    "Heading 6" : "标题 6",
    "Heading level 6" : "标题级 6",
    "Unordered list" : "无编号列表",
    "An item" : "一个项目",
    "Ordered list" : "编号列表",
    "First item" : "第一个项目",
    "Checklist" : "清单",
    "To-Do item" : "代办事项",
    "Blockquote" : "块引用",
    "Quoted text" : "引用文本",
    "Code block" : "代码块",
    "Some code" : "一些代码",
    "Link" : "链接",
    "Alt" : "Alt",
    "Footnote" : "脚注",
    "Emoji" : "表情符号",
    "emoji" : "表情符号",
    "Mention someone" : "提及某人",
    "name" : "名称",
    "Smart picker" : "智能选择器",
    "something" : "某些东西",
    "Action" : "操作",
    "Undo" : "撤销",
    "Redo" : "重做",
    "Toggle outline" : "切换大纲",
    "Open link" : "打开链接",
    "No link available to open" : "没有可打开的链接",
    "Cannot open links with unsafe protocols" : "无法打开具有不安全协议的链接",
    "Edit link" : "编辑链接",
    "Save changes" : "保存更改",
    "URL" : "URL",
    "Edit block formula" : "编辑块公式",
    "Edit inline formula" : "编辑行内公式",
    "LaTeX formula" : "LaTeX 公式",
    "Disabled because you are currently offline." : "已禁用，因为您当前已离线。",
    "Attachments cannot be created or uploaded because this file is shared from another cloud." : "无法创建或上传附件，因为此文件是从另一个云共享的。",
    "Upload from computer" : "从电脑上传",
    "Insert from Files" : "从文件插入",
    "Formatting help" : "排版帮助",
    "Link to file or folder" : "链接至文件或文件夹",
    "Link to website" : "链接至网站",
    "Update link" : "更新链接",
    "Open the Smart Picker" : "打开智能选择器",
    "{menuItemName}, \"{selectedSubMenuItemName}\" is selected" : "{menuItemName}, \"{selectedSubMenuItemName}\" 已选择",
    "Insert result" : "插入结果",
    "Insert" : "插入",
    "Nextcloud Assistant result copied" : "已复制 Nextcloud 助手结果",
    "Could not copy result to clipboard" : "无法复制结果至剪切板",
    "Translate" : "翻译",
    "Show assistant results" : "显示助手结果",
    "Assistant results" : "助手结果",
    "Nextcloud Assistant" : "Nextcloud助手",
    "Show result" : "显示结果",
    "Delete task" : "删除任务",
    "_%n word_::_%n words_" : ["%n 字"],
    "_%n char_::_%n chars_" : ["%n 字符"],
    "Remaining actions" : "重命名操作",
    "Editor actions" : "编辑器操作",
    "Formatting menu bar" : "格式化菜单栏",
    "Full width editor" : "全宽编辑器",
    "No emoji found" : "找不到任何表情",
    "No command found" : "找不到任何命令",
    "No user found" : "找不到任何用户",
    "No suggestion found" : "找不到任何建议",
    "Uploading attachments is disabled because the file is shared from another cloud." : "上传附件被禁用，因为该文件是从另一个云共享的。",
    "Upload" : "上传",
    "Insert Table" : "插入表格",
    "Smart Picker" : "智能选择器",
    "Copy code block" : "复制代码块",
    "Copy code" : "复制代码",
    "Code block options" : "代码块设置",
    "Code block language" : "代码块语言",
    "e.g. php, javascript, json…" : "比如 php, javascript, json…",
    "Source code" : "源代码",
    "Diagram" : "图标",
    "Both" : "都是",
    "Mermaid documentation" : "Mermaid 文档",
    "Failed to load" : "加载失败",
    "Could not find image in attachments list." : "在附件列表内无法找到任何图片。",
    "Delete this attachment" : "删除该附件",
    "Delete this image" : "删除该图片",
    "Left align column" : "左对齐列",
    "Center align column" : "居中对其列",
    "Right align column" : "右对齐列",
    "Sort ascending" : "升序排序",
    "Sort descending" : "降序排序",
    "Add column before" : "在之前新增列",
    "Add column after" : "在之后新增列",
    "Delete this column" : "删除该列",
    "Add row before" : "在之前新增行",
    "Add row after" : "在之后新增行",
    "Delete this row" : "删除该行",
    "Delete this table" : "删除该表格",
    "Add row below" : "在下方添加行",
    "Show folder description" : "显示文件夹描述",
    "Hide outline" : "隐藏大纲",
    "Show outline" : "显示大纲",
    "Done" : "完成",
    "Headings" : "标题",
    "Heading 2" : "标题 2",
    "Heading 3" : "标题 3",
    "Heading 4" : "标题 4",
    "Heading 5" : "标题 5",
    "Lists" : "列表",
    "To-Do list" : "代办事项",
    "Increase indentation" : "增加缩进",
    "Decrease indentation" : "减少缩进",
    "Blocks" : "区块",
    "Details" : "详情",
    "Inline math" : "行内数学",
    "Block math" : "块数学",
    "Info callout" : "信息标注",
    "Success callout" : "成功标注",
    "Warning callout" : "警告标注",
    "Danger callout" : "危险标注",
    "Question callout" : "问题标注",
    "Table" : "表格",
    "Annotations" : "注释",
    "Show annotations" : "显示注释",
    "Hide annotations" : "隐藏注释",
    "Insert link" : "插入链接",
    "Insert attachment" : "插入附件",
    "Insert emoji" : "插入 emoji",
    "Command" : "命令",
    "Ctrl" : "Ctrl",
    "Option" : "选项",
    "Suggestions" : "建议",
    "Formatting" : "格式",
    "Link a file" : "链接文件",
    "Start writing or type '/' to add…" : "开始撰写或输入“/”以添加…",
    "Select file or folder to link to" : "选择需要链接的文件或文件夹",
    "Choose" : "选择",
    "Choose {file}" : "选择 {file}",
    "Add folder description" : "添加文件夹描述",
    "\"{name}\" already exist!" : "\"{name}\" 已存在！",
    "Created \"{name}\"" : "创建 \"{name}\"",
    "Copied to the clipboard" : "已复制到剪切板",
    "Could not copy to the clipboard" : "无法复制到剪切板",
    "Front matter" : "前言",
    "A table was pasted into a table. Nested tables are not supported." : "表格被粘贴进另一个表格中，不支持嵌套表格。",
    "Link to this section" : "链接至本节",
    "text document" : "文本文档",
    "New text file" : "新建文本文件",
    "Overwrite the file and save the current changes" : "覆盖文件并保存当前更改",
    "Discard the current changes and load the latest version" : "放弃当前更改并加载最新版本",
    "Close outline view" : "关闭大纲视图",
    "Outline" : "大纲",
    "Not connected. Cannot update guest name." : "未连接。无法更新访客名称。",
    "Insert an attachment" : "插入附件",
    "Saving …" : "保存中 …",
    "Translate from {language}" : "翻译自 {language}",
    "Translate to {language}" : "翻译至 {language}",
    "Translation failed" : "翻译失败",
    "To translate individual parts of the text, select it before using the translate function." : "如果你需要翻译部分文字内容，请在使用翻译功能前选中该部分文字",
    "Translate from" : "翻译至",
    "Select language" : "请选择语言",
    "Text to translate from" : "需要翻译的文字",
    "to" : "到",
    "Translate to" : "翻译至",
    "Translated text result" : "文字翻译结果",
    "Replace" : "替换"
},
"nplurals=1; plural=0;");
