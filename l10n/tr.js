OC.L10N.register(
    "text",
    {
    "The file was uploaded" : "Dosya yüklendi",
    "The uploaded file exceeds the upload_max_filesize directive in php.ini" : "Yüklenen dosya php.ini dosyasındaki yüklenebilecek dosya boyutunu belirten upload_max_filesize değişkeninin değerini aşıyor",
    "The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form" : "Yüklenen dosya HTML formundaki yüklenebilecek dosya boyutunu belirten MAX_FILE_SIZE değişkeninin değerini aşıyor",
    "The file was only partially uploaded" : "Dosyanın yalnızca bir bölümü yüklendi",
    "No file was uploaded" : "Herhangi bir dosya yüklenmedi",
    "Missing a temporary folder" : "Bir geçici klasör eksik",
    "Could not write file to disk" : "Dosya diske yazılamadı",
    "A PHP extension stopped the file upload" : "Bir PHP eklentisi dosyanın yüklenmesini engelledi",
    "No file uploaded or file size exceeds maximum of %s" : "Herhangi bir dosya yüklenmedi ya da %s olan en büyük dosya boyutu sınırı aşıldı",
    "File not found" : "Dosya bulunamadı",
    "Nextcloud Text" : "Nextcloud Yazı",
    "Text document" : "Yazı belgesi",
    "Create new text file" : "Yeni yazı dosyası oluştur",
    "Editing session has expired. Please reload the page." : "Düzenleme oturumunun süresi dolmuş. Lütfen sayfayı yeniden yükleyin.",
    "{user} has mentioned you in the text document {node}" : "{user} kullanıcısı, {node} yazı belgesinde sizi andı",
    "This file cannot be displayed as download is disabled by the share" : "Paylaşım tarafından indirme özelliği kapatılmış olduğundan bu dosya görüntülenemiyor",
    "This document is read-only." : "Bu belge salt okunur.",
    "Read-only permission cannot save document changes. Please reload the page." : "Salt okunur izniyle belge değişiklikleri kaydedilemez. Lütfen sayfayı yeniden yükleyin.",
    "Readme" : "Benioku",
    "Text" : "Yazı",
    "📝 Collaborative document editing" : "📝 İş birlikli belge düzenleme",
    "** 📝 Collaborative document editing!**\n\n- **📝 Focused writing:** No distractions, only the formatting you need.\n- **🙋 Work together:** Share and collaborate with friends and colleagues, no matter if they use Nextcloud or not!\n- **💾 Open format:** Files are saved as [Markdown](https://en.wikipedia.org/wiki/Markdown), so you can edit them from any other text app too.\n- **✊ Strong foundation:** We use [🐈 tiptap](https://tiptap.scrumpy.io) which is based on [🦉 ProseMirror](https://prosemirror.net) – huge thanks to them!" : "** 📝 İş birlikli belge düzenleme!**\n\n- **📝 Odaklanmış yazım:** Dikkat dağıtan bir şey yok, yalnızca gerek duyacağınız biçimlendirme özellikleri var.\n- **🙋 Birlikte çalışma:** Nextcloud kullanıyor olsun ya da olmasınlar, tanıdık ve iş arkadaşlarınız ile paylaşarak iş birliği yapma!\n- **💾 Açık dosya biçimi:** Dosyalar [Markdown](https://en.wikipedia.org/wiki/Markdown), biçiminde kaydedildiğinden başka yazı uygulamalarında da düzenlenebilir.\n- **✊ Sağlam temel:** [🦉 ProseMirror](https://prosemirror.net) üzerine geliştirilmiş [🐈 tiptap](https://tiptap.scrumpy.io) uygulamasını kullanıyoruz. Onlara minnettarız!",
    "Your editing permissions have been revoked. The document is now read-only." : "Düzenleme izniniz kaldırıldı. Belge artık salt okunur.",
    "You now have edit permissions for this document." : "Artık bu belgeyi düzenleme izniniz var.",
    "Keep my local changes and overwrite the remote version" : "Yerel değişiklikler korunsun ve uzak sürümün üzerine yazılsın",
    "Discard my local changes and use the remote version" : "Yerel değişiklikler yok sayılsın ve uzak sürüm kullanılsın",
    "Comments" : "Yorumlar",
    "Previous comment" : "Önceki yorum",
    "Next comment" : "Sonraki yorum",
    "Close" : "Kapat",
    "Guest" : "Konuk",
    "guest" : "konuk",
    "Edit" : "Düzenle",
    "Delete" : "Sil",
    "Save" : "Kaydet",
    "Cancel" : "İptal",
    "Enter your name to comment" : "Yorum yapabilmek için adınızı yazmalısınız",
    "Add a comment…" : "Bir yorum ekle…",
    "Comment" : "Yorum",
    "Reply" : "Yanıtla",
    "Front matter changed" : "Açılış bölümü değiştirildi",
    "Image changed" : "Görsel değiştirildi",
    "Mention changed" : "Anma değiştirildi",
    "Mathematics changed" : "Matematik değiştirildi",
    "Footnote changed" : "Dipnot değiştirildi",
    "Divider changed" : "Ayırıcı değiştirildi",
    "Changed content" : "İçerik değiştirildi",
    "Paragraph" : "Paragraf",
    "Heading" : "Başlık",
    "List item" : "Liste ögesi",
    "Task" : "Görev",
    "Table" : "Tablo",
    "Table row" : "Tablo satırı",
    "Table cell" : "Tablo hücresi",
    "Image" : "Görsel",
    "Code block" : "Kod bloğu",
    "Quote" : "Alıntı",
    "Callout" : "Bildirim",
    "Details" : "Ayrıntılar",
    "Footnote" : "Dipnot",
    "Footnote reference" : "Dipnot başvurusu",
    "Front matter" : "Açılış bölümü",
    "Mathematics" : "Matematik",
    "Mention" : "Anma",
    "Link preview" : "Bağlantı ön izlemesi",
    "Added" : "Eklendi",
    "Removed" : "Kaldırıldı",
    "Changed" : "Değiştirildi",
    "Moved" : "Taşındı",
    "Content added" : "İçerik eklendi",
    "Content removed" : "İçerik kaldırıldı",
    "Content changed" : "İçerik değiştirildi",
    "No content" : "Herhangi bir içerik yok",
    "Block-level" : "Blok düzeyi",
    "Moved section" : "Bölüm taşındı",
    "Structure changed" : "Yapı değiştirildi",
    "{context} added" : "{context} eklendi",
    "{context} removed" : "{context} kaldırıldı ",
    "{context} changed" : "{context} değiştirildi",
    "Table column added" : "Tablo sütunu eklendi",
    "Table column removed" : "Tablo sütunu kaldırıldı",
    "Table column changed" : "Tablo sütunu değiştirildi",
    "Section: {section}" : "Bölüm: {section}",
    "Change pages" : "Sayfaları değiştir",
    "Previous" : "Önceki",
    "Showing changes {from}–{to} of {total}" : "Toplam {total} değişikliğin {from}–{to} arası görüntüleniyor",
    "Next" : "Sonraki",
    "Document start" : "Belge başlangıcı",
    "Block-level change; fine inline detail is unavailable" : "Blok düzeyinde değişiklik. İnce satır içi ayrıntısı kullanılamıyor",
    "_%n change_::_%n changes_" : ["%n değişiklik","%n değişiklik"],
    "_%n edit_::_%n edits_" : ["%n düzenleme","%n düzenleme"],
    "Reload" : "Yeniden yükle",
    "The file was overwritten. Your current changes cannot be auto-saved. Please choose how to proceed." : "Dosyanın üzerine yazıldı. Geçerli değişiklikleriniz otomatik olarak kaydedilemez. Lütfen nasıl ilerlemek istediğinizi seçin.",
    "The document could not be loaded. Please check your internet connection." : "Belge yüklenemedi. Lütfen internet bağlantınızı denetleyin.",
    "Reconnect" : "Yeniden bağlantı kur",
    "You've been disconnected from the server." : "Sunucu ile bağlantınız kesildi.",
    "This file is opened read-only as it is currently locked by {user}." : "Bu dosya {user} tarafından kilitlenmiş olduğundan salt okunur olarak açıldı.",
    "Insert below" : "Aşağıya ekle",
    "Click for options, hold to drag" : "Seçenekler için tıklayın, sürüklemek için basılı tutun",
    "Failed to update the guest name." : "Konuk adı güncellenemedi.",
    "Enter your name so other people can see who is editing" : "Adınızı yazın. Böylece diğer kişiler kimin düzenleme yaptığını görebilir",
    "Enter your name" : "Adınızı yazın",
    "submit" : "gönder",
    "you" : "siz",
    "edit" : "düzenle",
    "Uploading multiple attachments failed." : "Birkaç ek dosya yüklenemedi.",
    "Uploading attachment failed: {error}" : "Ek dosya yüklenemedi: {error}",
    "Uploading attachment failed." : "Ek dosya yüklenemedi.",
    "Failed to insert from Files" : "Dosyalar uygulamasından eklenemedi",
    "Failed to create attachment" : "Ek dosya oluşturulamadı",
    "Offline since {time}." : "{time} zamanından beri çevrim dışı.",
    "Not connected to the cloud." : "Bulut bağlantısı yok.",
    "Preview options" : "Ön izleme seçenekleri",
    "Text only" : "Yalnızca yazı",
    "Show link preview" : "Bağlantı ön izlemesini görüntüle",
    "Open in new tab" : "Yeni sekmede aç",
    "Copy link" : "Bağlantıyı kopyala",
    "Remove link" : "Bağlantıyı kaldır",
    "Active people" : "Etkin kişiler",
    "Offline" : "Çevrim dışı",
    "Offline, changes will be saved when online" : "Çevrim dışı. Çevrim içi olunduğunda değişiklikler kaydedilecek",
    "Saving …" : "Kaydediliyor…",
    "Saved" : "Kaydedildi",
    "Last saved {lastSave}" : "Son kaydedilme {lastSave}",
    "The document has been changed outside of the editor. The changes cannot be applied." : "Belge düzenleyici dışında başka bir uygulama ile değiştirilmiş. Değişiklikler uygulanamayacak.",
    "Unsaved changes" : "Kaydedilmemiş değişiklikler",
    "Save document" : "Belgeyi kaydet",
    "Last saved" : "Son kaydedilme",
    "Close table of contents" : "İçindekiler tablosunu kapat",
    "Pin table of contents" : "İçindekiler tablosunu sabitle",
    "Table of contents" : "İçindekiler tablosu",
    "Formatting and shortcuts" : "Biçimlendirme ve kısayollar",
    "Speed up your writing with simple shortcuts." : "Basit kısayol tuşlarını kullanarak yazmayı hızlandırabilirsiniz.",
    "Just type the Markdown syntax or use keyboard shortcuts from below." : "Aşağıda Markdown söz dizimini yazın ya da kısayol tuşlarını kullanın.",
    "Just type the Markdown syntax from below." : "Aşağıda Markdown söz dizimini yazın.",
    "Style" : "Biçem",
    "Syntax" : "Yazım",
    "Keyboard shortcuts" : "Kısayol tuşları",
    "New paragraph" : "Yeni paragraf",
    "Enter" : "Enter",
    "Hard line break" : "Mutlak satır sonu",
    "followed by" : "şunun izlediği",
    "Backspace" : "Geriye silme",
    "Shift" : "Shift",
    "Bold" : "Koyu",
    "Bold text" : "Koyu yazı",
    "Italic" : "Yatık",
    "Italicized text" : "Yatık yazı",
    "Underline" : "Altı çizili",
    "Underlined text" : "Altı çizili yazı",
    "Strikethrough" : "Üzeri çizili",
    "Mistaken text" : "Hatalı yazı",
    "Highlight" : "Vurgula",
    "Highlighted text" : "Vurgulanmış yazı",
    "Heading 1" : "1. Başlık",
    "Heading level 1" : "1. başlık düzeyi",
    "Heading 6" : "6. Başlık",
    "Heading level 6" : "6. başlık düzeyi",
    "Unordered list" : "Sıralı olmayan liste",
    "An item" : "Bir öge",
    "Ordered list" : "Sıralı liste",
    "First item" : "İlk öge",
    "Checklist" : "Denetim listesi",
    "To-Do item" : "Yapılacak iş ögesi",
    "Blockquote" : "Alıntı",
    "Quoted text" : "Alıntılanmış yazı",
    "Some code" : "Kod yazısı",
    "Link" : "Bağlantı",
    "Alt" : "Alt",
    "Emoji" : "Emoji",
    "emoji" : "emoji",
    "Mention someone" : "Birini an",
    "name" : "ad",
    "Smart picker" : "Akıllı seçici",
    "something" : "birşey",
    "Action" : "İşlem",
    "Undo" : "Geri al",
    "Redo" : "Yinele",
    "Toggle outline" : "Özeti aç/kapat",
    "Open link" : "Bağlantıyı aç",
    "No link available to open" : "Açılabilecek bir bağlantı yok",
    "Cannot open links with unsafe protocols" : "Güvenli olmayan iletişim kuralları kullanan bağlantılar açılamaz",
    "Edit link" : "Bağlantıyı düzenle",
    "Save changes" : "Değişiklikleri kaydet",
    "URL" : "Adres",
    "Changes" : "Değişiklikler",
    "Full documents" : "Tam belgeler",
    "Markdown source" : "Markdown kaynağı",
    "Before" : "Önce",
    "After" : "Sonra",
    "Before document" : "Belgeden önce",
    "After document" : "Belgeden sonra",
    "Detailed rendered comparison unavailable" : "Ayrıntılı görselleştirilmiş karşılaştırma kullanılamıyor",
    "Change {current} of {total}: {label}" : "{current} / {total} değişiklik: {label}",
    "No rendered changes" : "Görselleştirilmiş bir değişiklik yok",
    "Version comparison" : "Sürüm karşılaştırması",
    "Comparison view" : "Karşılaştırma görünümü",
    "Change navigation" : "Gezinmeyi değiştir",
    "Previous change" : "Önceki değişiklik",
    "Next change" : "Sonraki değişiklik",
    "Hide formatting-only changes" : "Yalnızca biçimlendirme ile ilgili değişiklikleri gizle",
    "{count} hidden" : "{count} gizli",
    "Show" : "Görüntüle",
    "All rendered changes are formatting-only and hidden by the current filter." : "Görselleştirilmiş tüm değişiklikler yalnızca biçimlendirme ile ilgili ve geçerli süzgeç tarafından gizleniyor.",
    "No rendered differences — Markdown syntax differs." : "Görselleştirilmiş bir fark yok. Markdown yazımları farklı.",
    "Open Markdown source" : "Markdown kaynağını aç",
    "No differences." : "Herhangi bir fark yok.",
    "Version to display" : "Görüntülenecek sürüm",
    "Source compares literal Markdown, so its change groups can differ from rendered changes." : "Kaynak, gerçek Markdown ile karşılaştırılır. Bu nedenle değişiklik grupları görselleştirilen değişikliklerden farklı olabilir.",
    "removed" : "kaldırıldı",
    "{count} trailing spaces" : "{count} sondaki boşluk",
    "No newline at end of file" : "Dosyanın sonunda yeni satır karakteri yok",
    "Markdown source comparison" : "Markdown kaynağı karşılaştırması",
    "Loading Markdown source differences…" : "Markdown kaynağı farklılıkları yükleniyor…",
    "Could not load Markdown source differences." : "Markdown kaynağı farklılıkları yüklenemedi.",
    "Retry" : "Yeniden dene",
    "Detailed source differences exceeded the safe processing limit." : "Ayrıntılı kaynak farklılıkları güvenli işleme sınırını aştı.",
    "Line endings changed: {before} → {after}" : "Satır sonları değiştirildi: {before} → {after}",
    "Source change {current} of {total}" : "{current} / {total} kaynak değişti",
    "Source version to display" : "Görüntülenecek kaynak sürümü",
    "Hide unchanged lines" : "Değiştirilmemiş satırlar gizlensin",
    "Show {count} unchanged lines" : "{count} değiştirilmemiş satırı görüntüle",
    "Show more unchanged lines" : "Diğer değiştirilmemiş satırları görüntüle",
    "Additional unchanged lines are hidden to keep the comparison responsive." : "Karşılaştırmayı uyumlu tutmak için değiştirilmemiş ek satırlar gizlendi.",
    "Lines {before} / {after}" : "{before} / {after} satır",
    "Complete Markdown source" : "Tüm Markdown kaynağı",
    "Complete Markdown source is shown because a detailed comparison is unavailable." : "Ayrıntılı bir karşılaştırma kullanılamadığı için tam Markdown kaynağı görüntüleniyor.",
    "Source preview was truncated to the display limit." : "Kaynak ön izlemesi ekran sınırında kesildi.",
    "Edit block formula" : "Blok formülü düzenle",
    "Edit inline formula" : "Satır arası formülü düzenle",
    "LaTeX formula" : "LaTeX formülü",
    "Disabled because you are currently offline." : "Çevrim dışı olduğunuzdan kapalı.",
    "Attachments cannot be created or uploaded because this file is shared from another cloud." : "Bu dosya başka bir buluttan paylaşıldığı için ek dosyalar oluşturulamıyor veya yüklenemiyor.",
    "Upload from computer" : "Bilgisayardan yükle",
    "Insert from Files" : "Dosyalar uygulamasından ekle",
    "Formatting help" : "Biçimlendirme yardımı",
    "Link to file or folder" : "Dosya ya da klasör bağlantısı",
    "Link to website" : "Site bağlantısı",
    "Update link" : "Bağlantıyı güncelle",
    "Open the Smart Picker" : "Akıllı seçiciyi aç",
    "{menuItemName}, \"{selectedSubMenuItemName}\" is selected" : "{menuItemName}, \"{selectedSubMenuItemName}\" seçilmiş",
    "Insert result" : "Sonuç ekle",
    "Insert" : "Ekle",
    "Nextcloud Assistant result copied" : "Nextcloud Yardımcısı sonucu kopyalandı",
    "Could not copy result to clipboard" : "Sonuç panoya kopyalanamadı",
    "Translate" : "Çeviri yapın",
    "Show assistant results" : "Yardımcı sonuçlarını görüntüle",
    "Assistant results" : "Yardımcı sonuçları",
    "Nextcloud Assistant" : "Nextcloud Yardımcısı",
    "Show result" : "Sonucu görüntüle",
    "Delete task" : "Görevi sil",
    "_%n word_::_%n words_" : ["%n sözcük","%n sözcük"],
    "_%n char_::_%n chars_" : ["%n karakter","%n karakter"],
    "Remaining actions" : "Yeniden adlandırma işlemleri",
    "Editor actions" : "Düzenleyici işlemleri",
    "Formatting menu bar" : "Menü çubuğu biçimlendiriliyor",
    "Full width editor" : "Tam genişlikte düzenleyici",
    "No emoji found" : "Herhangi bir emoji bulunamadı",
    "No command found" : "Herhangi bir komut bulunamadı",
    "No user found" : "Herhangi bir kullanıcı bulunamadı",
    "No suggestion found" : "Herhangi bir öneri bulunamadı",
    "Uploading attachments is disabled because the file is shared from another cloud." : "Dosya başka bir buluttan paylaşıldığı için ek dosyayı yükleme özelliği kullanılamıyor.",
    "Upload" : "Yükle",
    "Insert Table" : "Tablo ekle",
    "Smart Picker" : "Akıllı seçici",
    "Copy code block" : "Kod bloğunu kopyala",
    "Copy code" : "Kodu kopyala",
    "Code block options" : "Kod engelleme ayarları",
    "Code block language" : "Kod engelleme dili",
    "e.g. php, javascript, json…" : "php, javascript, json gibi…",
    "Source code" : "Kaynak kodu",
    "Diagram" : "Şema",
    "Both" : "İkisi de",
    "Mermaid documentation" : "Mermaid belgeleri",
    "Failed to load" : "Yüklenmemedi",
    "Could not find image in attachments list." : "Ek dosya listesinde görsel bulunamadı.",
    "Open attachment {name}" : "{name} ek dosyasını aç",
    "Delete this attachment" : "Bu ek dosyayı sil",
    "Open image {name}" : "{name} görselini aç",
    "Delete this image" : "Bu görseli sil",
    "Left align column" : "Sütunu sola hizala",
    "Center align column" : "Sütunu ortaya hizala",
    "Right align column" : "Sütunu sağa hizala",
    "Sort ascending" : "Artan sırala",
    "Sort descending" : "Azalan sırala",
    "Add column before" : "Önüne sütun ekle",
    "Add column after" : "Arkasına sütun ekle",
    "Delete this column" : "Bu sütunu sil",
    "Add row before" : "Üste satır ekle",
    "Add row after" : "Alta satır ekle",
    "Delete this row" : "Bu satırı sil",
    "Delete this table" : "Bu tabloyu sil",
    "Add row below" : "Aşağıya satır ekle",
    "Show folder description" : "Klasör açıklamasını görüntüle",
    "Image description changed" : "Görsel açıklaması değiştirildi",
    "Link target changed" : "Bağlantı hedefi değiştirildi",
    "Link changed" : "Bağlantı değiştirildi",
    "Link preview changed" : "Bağlantı ön izlemesi değiştirildi",
    "Task state changed" : "Görev durumu değiştirildi",
    "Heading level changed" : "Başlık düzeyi değiştirildi",
    "List start changed" : "Liste başlangıcı değiştirildi",
    "Code language changed" : "Kod dili değiştirildi",
    "Text direction changed" : "Yazı doğrultusu değiştirildi",
    "Table structure changed" : "Tablo yapısı değiştirildi",
    "Table alignment changed" : "Tablo hizalaması değiştirildi",
    "Callout type changed" : "Bildirim türü değiştirildi",
    "Details state changed" : "Ayrıntılar durumu değiştirildi",
    "Attribute changed" : "Öznitelik değiştirildi",
    "Inline code" : "Satır arası kod",
    "{formatting} changed" : "{formatting} değiştirildi",
    "Hide outline" : "Özeti gizle",
    "Show outline" : "Özeti görüntüle",
    "Done" : "Tamam",
    "Headings" : "Başlıklar",
    "Heading 2" : "2. Başlık",
    "Heading 3" : "3. Başlık",
    "Heading 4" : "4. Başlık",
    "Heading 5" : "5. Başlık",
    "Lists" : "Listeler",
    "To-Do list" : "Yapılacak iş listesi",
    "Increase indentation" : "Girintiyi artır",
    "Decrease indentation" : "Girintiyi azalt",
    "Blocks" : "Bloklar",
    "Inline math" : "Satır arası matematik ifade",
    "Block math" : "Blok matematik ifade",
    "Info callout" : "Bilgi bildirimi",
    "Success callout" : "Başarılı bildirimi",
    "Warning callout" : "Uyarı bildirimi",
    "Danger callout" : "Tehlike bildirimi",
    "Question callout" : "Soru bildirimi",
    "Annotations" : "Notlar",
    "Show annotations" : "Notları görüntüle",
    "Hide annotations" : "Notları gizle",
    "Insert link" : "Bağlantı ekle",
    "Insert attachment" : "Ek dosya ekle",
    "Insert emoji" : "Emoji ekle",
    "Command" : "Komut",
    "Ctrl" : "Ctrl",
    "Option" : "Seçenek",
    "Suggestions" : "Öneriler",
    "Formatting" : "Biçimlendirme",
    "Link a file" : "Bir dosya bağla",
    "Start writing or type '/' to add…" : "Yazmaya başlayın ya da eklemek için '/' kullanın…",
    "Select file or folder to link to" : "Bağlantı verilecek dosya ya da klasörü seçin",
    "Choose" : "Seçin",
    "Choose {file}" : "{file} seçin",
    "Add folder description" : "Klasör açıklaması ekle",
    "\"{name}\" already exist!" : "\"{name}\" zaten var!",
    "Created \"{name}\"" : "\"{name}\" oluşturuldu",
    "Copied to the clipboard" : "Panoya kopyalandı",
    "Could not copy to the clipboard" : "Panoya kopyalanamadı",
    "A table was pasted into a table. Nested tables are not supported." : "Bir tablo başka bir tablo içine yapıştırıldı. İç içe geçmiş tablolar desteklenmiyor.",
    "Link to this section" : "Bu bölüm bağlantısı",
    "text document" : "yazı belgesi",
    "New text file" : "Yeni yazı dosyası",
    "Overwrite the file and save the current changes" : "Dosyanın üzerine yaz ve geçerli değişiklikleri kaydet",
    "Discard the current changes and load the latest version" : "Geçerli değişiklikleri at ve son sürümü yükle",
    "Close outline view" : "Özet görünümünü kapat",
    "Outline" : "Özet",
    "Not connected. Cannot update guest name." : "Bağlı değil. Konuk adı güncellenemedi.",
    "Insert an attachment" : "Bir ek dosya ekle",
    "Saving …" : "Kaydediliyor …",
    "Translate from {language}" : "{language} dilinden çevir",
    "Translate to {language}" : "{language} diline çevir",
    "Translation failed" : "Çeviri yapılamadı",
    "To translate individual parts of the text, select it before using the translate function." : "Yazının ayrı bölümlerini çevirmek için çeviri özelliğini kullanmadan önce yazıyı seçin.",
    "Translate from" : "Kaynak dil",
    "Select language" : "Dil seçin",
    "Text to translate from" : "Çevrilecek yazı",
    "to" : "hedef dil",
    "Translate to" : "Hedef dil",
    "Translated text result" : "Çevrilmiş yazı",
    "Replace" : "Değiştir"
},
"nplurals=2; plural=(n > 1);");
